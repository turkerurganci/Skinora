...
**Versiyon: v3.8** | **Bağımlılıklar:** `01_PROJECT_VISION.md`, `02_PRODUCT_REQUIREMENTS.md`, `03_USER_FLOWS.md`, `04_UI_SPECS.md`, `10_MVP_SCOPE.md` | **Son güncelleme:** 2026-09-19 (**§3.3 "Enerji kilidi ayrı hesapta" satırı (#325)** — kilitli TRX sıcak cüzdanda durmaz, owner anahtarı çevrimdışı bir kilit hesabındadır; sıcak anahtar orada yalnız devretme/geri alma izniyle imzalar. Doğrulama düzeltmesiyle eklendi: ödeme enerjisi kilit hesabından sıcak cüzdana kalıcı devretmeyle gelir, bozuk kilit hesabı ayarı sidecar'ı açılışta durdurur; ayrıntı 08 §3.3.) · 2026-09-17 (**§3.3 TRON resource modeli koda hizalandı** — *"Deposit adreslerine TRX gönderilmez"* satırı hibrit karardan (2026-09-16) beri yanlıştı: depozite hesap açmak için 1 SUN ve kilit yetmediğinde yakacağı TRX gönderiliyor. Devretmenin transferin tamamı için yapıldığı, her adımın bloğa girişle onaylandığı, plan hesaplanamazsa sabit yedeğe düşüldüğü ve transferin kendisi revert ediyorsa hiçbir şeyin gönderilmediği yazıldı; ayrıntı 08 §3.3. #323 doğrulama bulgusu B4.) · 2026-08-22 (**T140 doğrulaması** — §5.3'ün *"koda hizalandı"* iddiası bağımsız ölçüldü ve **kısmen yanlış çıktı**, üç noktada düzeltildi: (a) `TransactionStatusChangedEvent`'in **DÖRDÜNCÜ bacağı** (`ITEM_DELIVERED → REFUNDED`, `SettlementVerificationJob.cs:384`) tabloda hiç yoktu ve alttaki not *"tek kasıtlı istisna ödeme onayıdır"* diyordu — oysa mutabakat geri alması da kendi domain olayıyla **birlikte** bu olayı yayınlıyor (aynı unit-of-work, `SaveChangesAsync` satır 397). Satır eklendi, not iki istisnayı da adıyla sayıyor. (b) Tüketici kolonu iki satırda kodla uyuşmuyordu: `TransactionCancelledEvent` "Blockchain" gösteriyordu — repoda öyle bir modül yok, gerçek tüketiciler `TransactionCancelledNotificationConsumer` + `TransactionCancelledRealtimeConsumer`; `SettlementReversalDetectedEvent` "Fraud" gösteriyordu — fraud işaretini bir tüketici değil **yayıncının kendisi** yazıyor. (c) `PaymentConfirmed` push'u "alıcıya" deniyordu; `SignalRTransactionRealtimePublisher.cs:52` işlem odasına gönderiyor, yani her iki taraf alır. **Ders T140'ın kendi dersinin aynısı, bu kez düzeltmenin üstünde:** bir hizalama iddiası, hizalamayı ölçen taramanın kapsamı kadar doğrudur — tablo olay **adları** ve **yayıncılar** için tam doğruydu, **tüketici** kolonu ve bacak sayımı için değildi.) · 2026-08-22 (**T140** — §5.3 domain event tablosu **koda hizalandı**: on bir satırın **yedisi** kodda karşılığı olmayan kavramsal adlar taşıyordu (`TransactionAcceptedEvent`, `SellerConfirmedReadyEvent`, `ItemDeliveredEvent`, `SettlementCompletedEvent`, `DeliveryReversedEvent`, `TransactionCompletedEvent`, `TransactionFlaggedEvent`) ve `PaymentReceivedEvent` satırı `DELIVERY_EXPECTED`’ı **yanlış olaya** bağlıyordu; satırda ayrıca iki custody kalıntısı vardı (*trade bağlantısı*, *§3.3 custody*). Tablo artık gerçek tip adlarını, yayıncılarını ve `TransactionStatusChangedEvent`’in üç ayrı bacağını taşıyor; ödeme onayının **neden** iki olay birden yayınladığı ve realtime çift-push’un bastırmayla değil **sahiplikle** önlendiği not olarak eklendi. Gerekçe ölçüldü: bu sapma, `DELIVERY_EXPECTED`’in dört görev boyunca hiç üretilememesinin bilgi ayağıydı. **Önceki (v3.4, 2026-08-17, T131)** — §4.4 geçiş tablosunun iki satırı netleşti: `PAYMENT_RECEIVED | timeout` satırına yanlış-teslimat imzasında **bekletme** ve **admin kararından sonra bekletmenin kalkması** yazıldı (`CLOSED` kaldırmaz — orada kimse bakmamıştır); `ITEM_DELIVERED | admin_resolve_refund` satırı ayrı gerekçe (`overrideReason`, 07 §9.30) şartını ve düzeltilmiş 02 §10.4 atfını taşıyor. **Yeni trigger yok** — bekletmenin kalkması mevcut `timeout` geçişini serbest bırakır.) · 2026-08-17 (T129 düzeltme turu — sweep tetikleyicisi satırı mutabakat kapısını (`SettlementVerifiedAt NOT NULL ∧ DeliveryReversedAt NULL`) da yazıyor; `ITEM_DELIVERED` state gate'i tek başına depoziti geri alma tespitinden günler önce boşaltırdı.) · 2026-08-16 (T129 — §4.2'de `complete` ve `delivery_reversed` satırları mutabakat kontrolünün **iki taraflı** olduğunu ve kapının payout/sweep üreticilerinde de okunduğunu yazar hâle getirildi. Geçiş kümesi değişmedi.) · 2026-08-10
...
| Hot wallet limiti | Operasyonel miktarda fon tutulur, fazlası cold wallet'a. Limit (`hot_wallet_limit`) admin tarafından belirlenir; ödeme sweep'ini beklediği için sıcak cüzdanda biriken şey **komisyondur**, limit ona göre küçük tutulur (owner kararı 2026-09-17: limit aşılınca admin elle aktarır, otomasyon yok). Hedef adres sabittir (yukarıdaki "Platform adresleri"), yani aktarım düğmesi parayı ancak soğuk cüzdana gönderebilir |
| Transfer limitleri | **İmzalayan serviste, ortam değişkeninde, fail-closed** (owner kararı 2026-09-17). (1) Tek gönderim tavanı `MAX_SINGLE_TRANSFER_USDT` — payout ve iadeye uygulanır, basamak kayması gibi hesap hatalarını geçirmez. (2) Sıcak cüzdanın son 24 saatlik çıkış toplamı için tavan `MAX_DAILY_OUTFLOW_USDT` — toplam zincirden okunur (TronGrid trc20 geçmişi, `only_from`), yani sidecar yeniden başlayınca sayaç sıfırlanmaz; ele geçirilmiş bir backend cüzdanı küçük adımlarla boşaltamaz. Soğuk cüzdana konsolidasyon **iki tavanın da dışındadır** — hedefi sabitlenmiştir, para platformdan çıkmaz. Tavan aşılırsa gönderim kalıcı reddedilir (400 → FAILED + alert); geçmiş okunamazsa geçici hata (502 → retry). Ayar yoksa **transfer yapılmaz** (08 §3.3) |
...
| Enerji kilidi ayrı hesapta | **Kilitli TRX sıcak cüzdanda durmaz** (owner kararı 2026-09-17). Hibrit enerji modelinde kilitlenen TRX, sıcak anahtarın arkasındaki en büyük değerdir — satışların parası T129'dan beri kendi depozit adreslerinde bekler, sıcak cüzdanda biriken komisyondur. Kilit kendi hesabındadır (`STAKE_ACCOUNT_ADDRESS`); **owner anahtarı çevrimdışıdır** ve sıcak cüzdanın anahtarı orada yalnız `DelegateResource` + `UnDelegateResource` yetkisi olan bir active permission ile listelidir. Sidecar devretmeyi ve geri almayı o hesap adına `Permission_id` ile imzalar (TronWeb `trx.sign` sahibi olmayan anahtarı reddeder; yol `multiSign`). **Kalan risk kabul edildi:** çalınan sıcak anahtar enerjiyi başka adrese devredebilir (mainnet en fazla ~30 gün) ama kilitli TRX'e dokunamaz — unstake, TRX transferi ve izin değiştirme `SIGERROR "Permission denied"` alır (Nile'da ölçüldü). Sidecar'ın **gönderdiği** TRX (hesap açma, yakma takviyesi, sabit yedek) yine sıcak cüzdandan çıkar: aynı izin transferi reddeder. **Ödeme enerjisi:** payout sıcak cüzdandan gider ve enerjiyi sıcak cüzdanın kendi havuzundan kullanır; kilit hesabı ödeme payını sıcak cüzdana bir kez ve kilitsiz devreder, devretme sürekli durur (owner kararı 2026-09-18, kod yok). Bozuk kilit hesabı ayarı (geçersiz adres ya da izin kimliği) sidecar'ı açılışta durdurur (owner kararı 2026-09-19). Kurulum, kalıcı devretme ve bant boyutlandırması DEPLOY_RUNBOOK §C.2; ayrıntı 08 §3.3 |
| Platform adresleri (hot / cold) | **Deployment yapılandırması, admin ayarı değil** (owner kararı 2026-09-16). `HOT_WALLET_ADDRESS` ve `COLD_WALLET_ADDRESS` hem backend'e hem blockchain sidecar'a aynı `.env`'den geçer; backend tarafında `IPlatformWalletAddressProvider` okur, sidecar **her sweep ve konsolidasyonda hedefi kendi değeriyle karşılaştırır** ve farklıysa imzalamaz (`DESTINATION_NOT_ALLOWED`). Admin panelinde yalnız görünür (07 §9.8 `isEditable: false`, yazma denemesi `SETTING_READ_ONLY`). **Neden:** eskiden `reconciliation.{hot,cold}_wallet_address` SystemSetting'leriydi ve `MANAGE_SETTINGS` yetkisi olan tek bir admin, biçim kontrolü/bekleme/ikinci onay olmadan değiştirebiliyordu — soğuk adresi değiştirip AD20 ile sıcak cüzdanı boşaltmak ya da sıcak adresi değiştirip mutabakatı biten her satışın depozitini kendine süpürtmek mümkündü (#312'nin ailesi). Adres değişikliği artık sunucu erişimi + iki servisin yeniden başlatılmasını gerektirir |
| Sweep mekanizması | Ödeme onaylandıktan sonra deposit adresindeki fon hot wallet'a sweep edilir (tek merkezi adres) |
| Sweep tetikleyicisi | `ITEM_DELIVERED` state gate'i **ve** mutabakat kapısı (`SettlementVerifiedAt NOT NULL ∧ DeliveryReversedAt NULL`) — sweep, alıcı-iade penceresi (sweep öncesi iade deposit adresinden çıkar) kapandıktan sonra başlar. Eager `PaymentReceivedEvent` tetiği depoziti erken boşaltıp yaygın "ödeme-sonrası-iptal" iadesini bozacağı için ertelendi (WP3 owner kararı 2026-06-15; `SweepQueueJob`). Mutabakat kapısı aynı gerekçenin devamıdır: mutabakat süresi boyunca iade hâlâ mümkündür (trade geri alma → REFUNDED) ve o iade de deposit adresinden çıkar, dolayısıyla depozit iadenin çekilebileceği yerde kalmalıdır. `ITEM_DELIVERED` tek başına yeterli olsaydı sweep, geri alma tespitinden günler önce depoziti boşaltırdı (T129, 06 §3.5 — aynı çift COMPLETED guard'ı ve payout kuyruğunda da okunur) |
| Payout / iade kaynağı | Satıcıya ödeme **sweep sonrası hot wallet'tan** çıkar — ve "sonrası" artık kuyruk düzeyinde zorunludur: `SellerPayoutQueueJob` işlemin kendi SWEEP satırı `CONFIRMED` olana kadar payout satırı üretmez (owner kararı 2026-09-17). Böylece sıcak cüzdan yalnız o işlem için **eline geçmiş** parayı öder; işletme bakiyesi (float) gerekmez ve orada biriken şey komisyondur. Bedeli: sweep kalıcı başarısızsa payout da bekler — mevcut sweep-hatası alarmı bunu zaten operatöre bildirir. Alıcıya iade ise **sweep öncesi (genelde teslim öncesi iptal/timeout) deposit adresinden** çıkar — aşağıdaki "Sweep öncesi refund" satırına bakınız (WP2/WP3) |
...
| Sweep hatası | Sweep başarısız olursa retry (exponential backoff, 3 deneme). Tüm denemeler başarısızsa admin'e alert ve **payout bekler** (yukarıdaki "Payout / iade kaynağı"). Teslimat öncesi iade zaten depozitten çıktığı için sweep'e bağlı değildir. **Not (2026-09-17):** bu satır daha önce "payout deposit adresinden doğrudan gönderilir (fallback)" diyordu; öyle bir yol kodda hiç olmadı — `SELLER_PAYOUT` satırı şema düzeyinde depozite bağlanamaz (`PaymentAddressId` NULL zorunlu, 06 §3.8), dolayısıyla iddia düzeltildi |
...
| TRX prefunding | Kaynak kararı **hibrit** (proje sahibi, 2026-09-16 — 08 §3.3). Depozite iki durumda TRX gönderilir: (1) **hesap açma** — yalnız TRC-20 almış bir adres zincirde hesap değildir ve ona devretme de ondan gönderim de reddedilir; sweeper önce 1 SUN gönderir (açılış ücreti 1,1 TRX sweeper'dan çıkar); (2) **yakma** — kilit transferin tamamını karşılayamıyorsa depozite transferin yakacağı TRX gönderilir. Adres başına TRX takibi gerekmez: miktar her transfer için zincirden hesaplanır |
| Energy delegation | Sweeper account TRX kilitleyerek energy üretir. Kilit transferin **tamamını** karşılayabiliyorsa gönderimden önce depozite o kadar energy devredilir ve devretme bloğa girip depozitte görünmeden transfer yayınlanmaz; geri alma, transfer bir bloğa girdikten sonra yapılır (08 §3.3) |
| Fallback | Kilit transferin tamamını karşılayamıyorsa ya da devredilen energy depozitte görünmezse → depozite yakacağı TRX gönderilir (kısmi devretme yapılmaz). Plan hiç hesaplanamazsa sabit yedek TRX (`SWEEP_TRX_FALLBACK_SUN`) gönderilir. Transferin kendisi simülasyonda revert ediyorsa hiçbir şey gönderilmez ve yayınlanmaz; yeniden denenebilir hata döner (08 §3.3) |
...
| PAYMENT_RECEIVED | timeout | CANCELLED_TIMEOUT | Para alıcıya iade — **sorumlu: satıcı**. İptal uygulanmadan önce son bir teslimat doğrulaması çalışır; kanıt bulunursa geçiş `deliver_item`'a döner. **Yanlış-teslimat imzası bulunursa geçiş bekletilir** (02 §9.2 sessiz iptali yasaklar) ve satır admin'e eskale edilir; **admin dispute'u karara bağladıktan sonra bekletme kalkar** ve geçiş uygulanır (T131 — 02 §10.4, 03 §6.4). Sistemin kendi otomatik çözümü (`CLOSED`) bekletmeyi kaldırmaz: orada kimse bakmamıştır |
...
| ITEM_DELIVERED | admin_resolve_refund | REFUNDED | Para alıcıya iade. **Item geri alınamaz** — platform item'a erişemez, zarar geri alma imkânı olmadan satıcıya devredilir (02 §10.4). Bu geçiş teslimatın kanıtlandığı bir işlemde yapıldığı için **ayrı bir gerekçe** ister (`overrideReason`, 07 §9.30 AD29 — T131) |
...
| `PaymentMonitorStartRequestedEvent` | `TransactionReadinessService` | `PaymentMonitorStartDispatcher` (Transactions) → Blockchain sidecar | Depozit adresinin aktif izleyicisini kur (T139). Tüketici sınıfı Transactions modülünde yaşar; "Blockchain" burada yan etkinin gittiği yerdir, tüketicinin bulunduğu modül değil |
| `PaymentReceivedEvent` | `AmountValidationService`<br>`.AdvanceStateMachineAsync` | Notification, Realtime | Satıcıya "ödeme alındı" (`PAYMENT_RECEIVED`) + `PaymentConfirmed` push. Push **işlem odasına** gider (`SendAsync(TransactionId, …)`), yani odaya katılmış her iki taraf alır. **Para bilgisini taşır** (tutar / token / txHash) |
...
| `SettlementReversalDetectedEvent` | `SettlementVerificationJob` | Notification | Trade geri alınmış → admin/taraf bildirimi. **Fraud işareti bir tüketiciden gelmez** — yayıncının kendisi aynı unit-of-work'te yazar; iade `PaymentRefundToBuyerRequestedEvent` ile ayrı yürür |
| `TransactionStatusChangedEvent`<br>(→ `REFUNDED`) | `SettlementVerificationJob` | Realtime | Mutabakatta geri alma tespit edildi → taraf ekranları mutabakat sayacını göstermeye devam etmesin. `SettlementReversalDetectedEvent` ile **aynı** unit-of-work'te yayınlanır (aşağıdaki nota bak) |
...
| `TransactionCancelledEvent` | `TransactionCancellationService`<br>`AdminTransactionService` | Notification, Realtime | İptal bildirimleri + durum push'u. **Para iadesini bu olay sürmez** — iade ayrı `PaymentRefundToBuyerRequestedEvent` ile yürür. **Item iadesi yoktur** (aşağıdaki v3.0 notu) |
...
> **`TransactionStatusChangedEvent` neden dört ayrı satırda:** jenerik olaydır, kendi domain olayı olmayan geçişleri taşır ve tüketicileri `ToStatus`'a göre dallanır. Kendi domain olayı olan geçişler (kabul, payout, iptal, timeout, dispute, flag, emergency-hold) bu olayı yayınlamaz — çift push olmasın diye. **İki kasıtlı istisna vardır**, ikisinde de ikinci olay var çünkü birincinin hiçbir tüketicisi o işi yapmıyor: **(1) ödeme onayı** — hem `PaymentReceivedEvent` hem bu olay yayınlanır, çünkü ikisi ayrık tüketicilere farklı olgular taşır (para ↔ durum çifti) ve `DELIVERY_EXPECTED`'ın tek üreticisi bu olaydır; **(2) mutabakat geri alması** (`ITEM_DELIVERED → REFUNDED`) — `SettlementReversalDetectedEvent`'in yalnız bildirim tüketicisi vardır, durum push'u bu olaydan gelir. Realtime çift-push'u her ikisinde de bastırmayla değil **sahiplikle** önlenir: kardeş olayın tüketicisi kendi payload'ını, jenerik relay yalnız `StatusChanged`'i push eder (T140).
...
*Skinora — Technical Architecture v3.7*
