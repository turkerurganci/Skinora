...
> **Kontrol 10'un ödeme bacağı elle kurulur (ölçülmüş boşluk, prova kısayolu DEĞİL).** Backend deposit adresini türetir (`api/wallet/derive`) ve `PaymentAddress.MonitoringStatus`'ü `ACTIVE` yapar, ama blockchain sidecar'ın **ödeme izleyicisini kurmaz**: `IBlockchainSidecarClient` yalnız `derive` · `monitor/post-cancel-start` · `monitor/post-cancel-stop` · `wallet/balances` · `transfer/cold-wallet` taşır — sidecar'ın T71 ucu `POST /api/monitor/start`'ı çağıran **hiçbir backend kodu yoktur** ve `MonitorRegistry.start` yalnız o route'tan erişilebilir. İzleyici kurulmadan Nile transferi `payment-detected` **üretmez** ve prova `SELLER_CONFIRMED`'da sessizce durur. Provada izleyiciyi elle kurun:
>
> ```bash
> curl -X POST http://localhost:5200/api/monitor/start \
>   -H "X-Internal-Key: $INTERNAL_KEY" -H 'Content-Type: application/json' \
>   -d '{"address":"<deposit-adresi>","paymentAddressId":"<payment-address-id>",
>        "transactionId":"<transaction-id>","expectedContract":"<USDT-kontrat-adresi>",
>        "expectedSymbol":"USDT"}'
> ```
>
> Bu satır bir kolaylık değil, **bağlanmamış bir caller'ın kaydıdır** — sınıfı `T81-PriceConsumerWireup` / `T50-OutageFreezeCallers` ile aynı. Kayıt: `DEFERRED_BACKLOG` → `T133b-PaymentMonitorUnarmed`.
