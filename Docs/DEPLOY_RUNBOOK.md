...
> **§G.4 kontrolleri 1–9 + satıcı uygunluğu 2026-08-29'da koşuldu (prova provası — alıcı bacağı MA beklemesi yüzünden koşulamadı).** Sonuçlar: `:8080/health` 200 · `SystemSetting bootstrap complete`, fail-fast yok · `:5100/health` `steam-api reachable` · steam sidecar `listening`, bot/credential satırı yok · blockchain sidecar `network: nile` · `GET /admin/settings` **63 satır / 0 null** · `GET /steam/inventory` **gerçek CS2 envanteri** (`Tec-9 | Groundwater`, `assetId 18514036356`). **Kontrol 7 (tarayıcıda Steam girişi) bu turda koşulmadı** — JWT enjeksiyon yolu kullanıldı, o yol OpenID callback'ini atlar.
>
