...
**Kalan gün sayısı yalnız satıcı kapısında taşınır.** `GET /transactions/eligibility`, `STEAM_ACCOUNT_TOO_NEW` sebebinin yanında `steamAccountRemainingDays` alanını da döndürür (07 §7.3) ve arayüz bunu banner'da gösterir (04 §7.2). Diğer iki sebepte alan **hiç yazılmaz**. Alıcı kapılarında sayı ayrı bir alanla taşınmaz: oradaki sunucu mesajı zaten sayıyı içerir, istemci ise metni **hata koduna** göre seçer, dolayısıyla sayıyı ek bir alanla taşımak hata zarfının sözleşmesini (07 §2.4 `details` yalnız doğrulama hataları içindir) değiştirmeyi gerektirirdi.

**Satıcı kapısı, daha ucuz bir kural zaten reddetmiş olsa bile Steam'e sorar.** Ucun sözleşmesi **eksiksiz sebep listesidir**: iki engeli olan satıcı ikisini birden görür, birini çözüp diğerine çarpmaz. Kota da bundan kazanmazdı — sorgu temiz sonucu 24 saat önbelleğe alır, kısıtlı sonucu hiç almaz; yani kotayı harcayan hesaplar, sıralama değişse de harcamaya devam ederdi. (Alıcı kabul kapısı bunun tersini yapar ve orada erken çıkış testle pinlidir: o uç tek bir karar döndürür, liste değil.)
...
| `POST /wallet/getaccountresource` | Hesabın harcanabilir Energy/Bandwidth durumu + ağ Energy/TRX oranı | Gönderim öncesi gas fee tahmini (§3.1a) — kontrat sahibinin kalan enerjisi dahil (adım 2) |
| `GET /wallet/getchainparameters` | Zincir birim fiyatları (`getEnergyFee`, `getTransactionFee`) | Gönderim öncesi gas fee tahmini (§3.1a) |
| `POST /wallet/getcontract` | Kontratın `consume_user_resource_percent` ayarı, `origin_energy_limit`'i ve sahibi (`origin_address`) — enerjiyi kim öder | Gönderim öncesi gas fee tahmini (§3.1a adım 2) |
| `POST /wallet/gettransactioninfobyid` (full node) | Bir işlemin bloğa girip girmediği — blok numarası; hiçbir blok tutmuyorsa boş gövde `{}` | Deposit kaynak akışında her adımın onayı (§3.3 "Blok onayı") |
...
2. **Enerjiyi kim ödeyecek?** Zincirin gerçekte uyguladığı bölüşüm hesaplanır (java-tron `ReceiptCapsule.payEnergyBill`): **sahibin payı = min(⌊toplam × (100 − yüzde) / 100⌋, sahibin kalan enerjisi, `origin_energy_limit`)**, gerisini çağıran öder. Yüzde (`consume_user_resource_percent`), `origin_energy_limit` ve sahip (`origin_address`) `getcontract`'tan, sahibin kalan enerjisi sahibin kendi `getaccountresource`'undan okunur. **Yüzde tek başına cevap değildir:** mainnet Tether 30 der, ama sahibinin kalan enerjisi 0'dır (`EnergyLimit` 9.203 = `EnergyUsed` 9.203) ve 2026-09-17'de ardışık örneklenen 5 bloktaki **542 USDT çağrısının 542'sinde** çağıran toplam enerjinin **%100,00**'ünü ödedi (sahip bloğu başına en fazla 1 enerji). Yalnız yüzdeyi okuyan tahmin mainnet kullanıcısından gerçek maliyetin %30'unu kesiyordu. Yüzde alanı 0 iken cevapta **hiç dönmez**, yani yokluğu "sahip öder" demektir — ama `getcontract` kontrat olmayan bir adrese de **boş gövdeyle** cevap verir (Nile'da ölçüldü) ve o boşluk meşru sıfırdan ayırt edilemez; bu yüzden kimliksiz gövde cevap değil **başarısız prob** sayılır. Provalarda kullanılan Nile test USDT'si (`TXYZop…`) sahibinin ödediği bir kontrattır (yüzde 0, sahibin ~197,5 milyon enerjisi var) ve **provadaki `fee: 0`'ın gerçek sebebi budur**. Sahibin ödediği pay kullanıcıdan kesilmez; kimsenin ödemediği bir maliyeti faturalamak olurdu. Kontrat ya da sahibin hesabı okunamazsa ya da sahip bildirilmezse **"sahip hiçbir şey ödemez"** varsayılır — tahmin yalnızca büyür, asla küçülmez.
3. **Platform ne kadar enerji getirebilir?** Payout'ta hot wallet doğrudan gönderdiği için havuzunun tamamı geçerlidir (kilit ayrı bir hesaptaysa, o hesabın sıcak cüzdana **kalıcı** devrettiği ödeme payı bu havuzdadır — §3.3 "Kilidin sahibi ve imzacısı"); eksik kalanı zincir yakar. İadede gönderen bir deposit adresidir ve iade, gönderimin izleyeceği **aynı planla** fiyatlanır (`DelegationPlanner`, aşağıdaki "Deposit adresi resource modeli"): plan, gönderim gibi **transferin tamamı** için yapılır; kilidi tutan hesap bu açığın **tamamını** devredebiliyorsa enerji kredisi tam, devredemiyorsa **sıfır**. Kilidi tutan hesap, gönderimin devrettiği hesaptır (ayrı kilit hesabı; yoksa hot cüzdan) ve tahmin onu teslim akışının **kendisine sorar** — ayrı okuyan bir tahmin, kilit taşınınca sıcak cüzdanda devredilecek bir şey bulamayıp yakma planlıyor ve alıcının iadesinden olmayan bir yakma kesiyordu (#325 doğrulaması, 2,16–4,38 USDT) — kısmi kredi yoktur, çünkü gönderim kısmi devretme yapmaz. Yakma yolunda kullanıcıdan yalnız zincirin depozitten gerçekten yakacağı kadar (adım 2'nin payı − depozitin kendi enerjisi) kesilir; sahibin payı için gönderilen TRX yakılmaz, depozitte kalır ve platformun maliyetidir. Kilitli bir hot cüzdanın büyük **havuzu** tek başına hiçbir şey ifade etmez; sayılan, bu transfer için devredilebilecek miktardır. Hiç TRX almamış bir deposit adresi 0 bant genişliği bildirir, ama gönderimden önce açıldığı için ücretsiz günlük banttan (600 bayt) yararlanır ve tahmin bunu hesaba katar.
...
**Tahmin, gönderilecek token ile alınır — beklenen token ile değil.** Adım 1'in simüle ettiği transfer, gönderimin yayınlayacağı transferin **aynısı** olmalıdır: gönderen, alıcı, tutar **ve token**. İade ailesinin çoğunda bu beklenen stablecoin'dir (ödeme o token'la gelmiştir). **Yanlış token iadesi** tek istisnadır: 06 §3.8 satırın `Token` alanında beklenen stablecoin'i tutar, ama gönderim deposit adresine **fiilen gelen** token'ı `ActualTokenAddress`'ten çözüp yayınlar. Tahmin de aynı çözümle alınır. Beklenen token'la alınan bir tahmin, deposit adresinde hiç olmayan bir token'ın transferini simüle ederdi — iki kontrat hem enerji tüketiminde hem `consume_user_resource_percent` ayarında (adım 2) ayrışır, sapma bir transfer mertebesindedir (~65.000 Energy × 100 SUN = 6,5 TRX ≈ 2,1 USD) ve yönü belirsizdir (`WrongTokenRefundFeeEstimatesExpectedToken`). Kontrat hiçbir izinli stablecoin'e çözülemezse (yalnız backend ve sidecar allowlist'leri ayrışırsa) gönderim iadeyi yayınlamadan terminal olarak düşürür; fiyatlanacak gerçek bir transfer olmadığı için tahmin beklenen token'la alınır.
...
>
> **Enerji kilitleme oranı ağ genelinde bir değerdir ve sabit değildir:** `TotalEnergyLimit / TotalEnergyWeight`. Mainnet'te **~9,57** (2026-08-29) ve **~9,52** (2026-09-16) Energy / kilitlenen TRX, Nile'da **~73,7**. Bu yüzden devredilecek miktar bir ayar değil, **her transfer için** güncel orandan hesaplanır (aşağıdaki model). Eski sabit 200 TRX'lik delegasyon mainnet'te bir transferin **~%3'ünü** karşılıyordu ve testnet provası bunu **gizliyordu**.

**Enerji kararı — HİBRİT (proje sahibi, 2026-09-16).** Beklenen günlük taban hacim kadar TRX ENERGY için kilitlenir; kilidin karşılayamadığı her transfer yakar. Kilit 2026-09-17'den beri ayrı bir **kilit hesabında** durur ve ödeme payı oradan sıcak cüzdana **kalıcı** devredilir (2026-09-18 — aşağıdaki "Kilidin sahibi ve imzacısı"). Ölçülen maliyetler (2026-09-16, TRX 0,336 $, `getEnergyFee` 100 SUN):

| Satış başına | Yakma | Hibrit (kilit yeterliyse) |
|---|---|---|
| Deposit hesabını açma | 1,1 TRX | 1,1 TRX |
| Sweep enerjisi (64.285) | 6,43 TRX | 0 (devredilir) |
| Payout enerjisi (64.285 / 130.285) | 6,43–13,03 TRX | 0 (sıcak cüzdanın kendi havuzu: kilit hesabının ona kalıcı devrettiği ödeme payı — kilit hesabı yoksa kendi kilidi) |
| Bant genişliği — ücretsiz kota bittiyse (yakma: yakma parasının gönderimi + payout; hibrit: devretme + geri alma + payout — kilit hesabı ayrıysa devretme ile geri almanın bandı **onun** hesabından çıkar, DEPLOY_RUNBOOK §C.2) | ~0,6 TRX | ~0,9 TRX |
| Yakma payı — sweep'e gönderilen TRX'in yakılmayan %10'u depozitte kalır, geri toplanmaz (64.285 × 100 SUN × 0,1; 2026-09-17'de eklendi) | ~0,64 TRX | 0 |
| **Toplam** | **~15,2–21,8 TRX ≈ 5,1–7,3 $** | **~2 TRX ≈ 0,67 $** |

Günde bir satışlık kapasite ≈ satış başına enerji ÷ oran = 128.570–194.570 ÷ 9,52 ≈ **13.500–20.400 TRX** kilit. Kilitli TRX harcanmaz (14 gün beklemeyle geri alınır); **kullanılan** her kilitli TRX yılda ≈ 9,52 × 365 × 100 SUN ≈ 0,35 TRX yakmayı önler. Kullanılmayan enerji birikmez. Kilit boyutlandırma adımı: `DEPLOY_RUNBOOK` §C.2.
...
| **Hesap açma** | Yalnız TRC-20 almış bir adres zincirde **hesap değildir**: ona devretme (`Account[…] not exists`) ve ondan gönderim (`account […] does not exist`) doğrulamada reddedilir (Nile'da ölçüldü, 2026-09-16). Sweeper önce **1 SUN** gönderir; hesap açma ücretini (1 TRX + 0,1 TRX bant) **gönderen** öder. Açılan hesap günlük 600 bayt ücretsiz bant alır | 2026-09-16 ölçümü |
| **Plan** | Transfer simüle edilir; açık = **transferin tamamı** − depozitin mevcut enerjisi. Kontrat sahibinin payı boyutlandırmada **hesaba katılmaz** (proje sahibi kararı, 2026-09-17): o pay, kontratın diğer çağıranlarının aynı blokta tükettiği bir havuzdan gelir; ona göre boyutlanan transfer havuz plan ile blok arasında boşalınca zincirde enerjisiz kalıp düşer, fazla boyut ise kalıcı maliyet değildir (devretme geri alınır, TRX depozitte kalır). Devredilecek miktar = ⌈açık ÷ oran × 1,1⌉ TRX (tam TRX'e yukarı yuvarlanır, dolayısıyla en az 1 TRX). **Hep ya hiç:** kilidi tutan hesabın — ayrı kilit hesabı, tanımlı değilse sıcak cüzdan (aşağıda "Kilidin sahibi ve imzacısı") — devredebildiği (`getcandelegatedmaxsize`) bu miktarı karşılıyorsa devredilir, karşılamıyorsa hiç devredilmez | `DelegationPlanner`, 2026-09-17 |
| **Devretme yolu** | Devret → işlem bir bloğa girene ve depozitin enerjisi **transferin tamamına** ulaşana kadar bekle → transfer → **transfer bir bloğa girene kadar bekle** → geri al. Enerji gelmezse (devretme bloğa girmez ya da oran kaymıştır) geri alınır ve yakma yoluna geçilir: kısmi enerjiyle gönderilen transfer zincirde başarısız olur ve arkasından hiçbir yedek çalışmaz | 2026-09-16, 2026-09-17 |
| **Yakma yolu** | Depozite yakacağı kadar TRX gönderilir (⌈açık × enerji ücreti × %110⌉ + gerekirse bant, eldeki bakiye düşülür), işlemin bloğa girmesi ve bakiyede görünmesi beklenir, sonra transfer. %10 pay depozitte kalır (~0,64 TRX / 64.285 enerji, ~1,30 TRX / 130.285); sahibin ödediği bir kontratta gönderilen TRX'in tamamı yakılmadan kalır (Nile'da her transferde ~1,6 TRX) | 2026-09-16, 2026-09-17 |
| **Blok onayı** | Her adım — hesap açma, devretme, TRX gönderimi ve geri almadan önce transferin kendisi — `gettransactioninfobyid` bir blok numarası döndürene kadar beklenir. Hesap okumaları (`getaccount`, `getaccountresource`) düğümün **bekleyen** durumunu gösterir, onay değildir: 2026-09-16 Nile koşusunda devretme, "enerji geldi" kontrolünden **sonra** oluşturulan transfer ve geri alma **aynı bloğa** girdi (71.019.348 #2/#5/#6) — sıraları yalnız blok üreticisine varış sırasıydı. Bloktan sonra hesap okumasının da bloğu göstermesi beklenir (TronGrid istekleri farklı düğümlere dağıtır). Bekleme iki boydadır: vazgeçmenin yalnız "sonra tekrar dene" demek olduğu adımlarda (hesap açma, TRX gönderimi; geç inen işlem bir sonraki denemede görülür) ~30 sn; erken vazgeçmenin devretmeyi sahipsiz bırakabileceği ya da transferin önüne geçirebileceği adımlarda (devretme, geri alma öncesi transfer) düğümün her işleme koyduğu **60 sn'lik son kullanma süresinden** uzun, ~75 sn — o süreden sonra hiçbir blok tutmayan işlem artık inemez | 2026-09-17 |
| **Yayın penceresi** | Sidecar, çağrının başından **150 sn** geçtiyse transferi yayınlamaz: devretmeyi geri alır ve yeniden denenebilir hata (`TRANSFER_WINDOW_ELAPSED`) döner. Backend'in transfer çağrısı zaman aşımı **300 sn**'dir (`BlockchainSidecar:TransferTimeoutSeconds`) — bu pencere + transferin ~75 sn'lik blok beklemesini kapsar. Aksi hâlde backend vazgeçip yeniden denerken sidecar ilk transferi yayınlamış olur, deneme boşalmış depozitte çalışır ve ilk transfer hiç kaydedilmez. Aynı sebeple gönderim işi (`OutgoingTransferDispatchJob`) aynı anda tek tur çalışır; durum sorgusu kendi 30 sn'lik bütçesini korur | 2026-09-17 |
| **Yedek** | Plan hiç hesaplanamazsa (zincir probu arızası) sabit `SWEEP_TRX_FALLBACK_SUN` (15 TRX) gönderilir — en pahalı transfer 13,03 TRX + bandı 0,35 TRX (hesap açma ücreti yedekten değil, 1 SUN'u gönderen sweeper'dan çıkar). **Hesap açma adımındaki hata** (hesap probu, 1 SUN gönderimi ya da bloğa girmemesi) yedeğe gitmez: yeniden denenebilir hata döner ve backend çağrıyı tekrar dener. **Transferin kendisi revert ediyorsa** yedek de gitmez (proje sahibi kararı, 2026-09-17): simülasyonu düğüm **çalıştırıp** başarısız bulduysa (`result.result: true` + `REVERT opcode executed` ya da başarısız `ret` — mainnet'te ölçüldü) bu bir prob arızası değil, transferin zincirde düşeceği cevabıdır. Depozite TRX gönderilmez, transfer yayınlanmaz, yeniden denenebilir `DEPOSIT_TRANSFER_WOULD_REVERT` döner; backend 1, 5 ve 15 dk sonra dener, sonra satır `FAILED` olur ve admin uyarısı gider. En olası sebep, önceki bir denemenin token'ı göndermiş ama backend'in bunu kaydedememiş olmasıdır — hata metni iadeyi yeniden başlatmadan önce depozitin giden transferlerine bakılmasını söyler. Düğümün çağrıyı **reddetmesi** (`OTHER_ERROR`, `CONTRACT_VALIDATE_ERROR` — `result: true` taşımaz) revert sayılmaz, prob arızasıdır. Simülasyonun kendi hatası diğer probların hatasından önce gelir; aynı anda başka bir okuma düşse de revert yedeğe dönüşmez. Nile'da ölçüldü (2026-09-17, derlenmiş kod): 0 USDT tutan depozit #9010'dan 1 USDT → `DEPOSIT_TRANSFER_WOULD_REVERT`, hiçbir işlem yayınlanmadı, depozitin TRX bakiyesi değişmedi | 08 §3.3, 2026-09-17 |
...
| **Kilidin sahibi ve imzacısı** | Kilit ayrı bir hesapta durur (`STAKE_ACCOUNT_ADDRESS`, owner kararı 2026-09-17). Devretme ve geri alma o hesap adına kurulur: işleme `Permission_id` yazılır ve sıcak cüzdanın anahtarıyla `multiSign` ile imzalanır. **`trx.sign` bu yolu reddeder** — *"Private key does not match address in transaction"* (2026-09-18'de ölçüldü). İzinsiz imza zincirde `SIGERROR … is not contained of permission` alır. **Ama akış bu reddi yakalayıp yakmaya geçer:** biçimce geçerli ama yanlış bir izin kimliğinde operatör yalnız WARN satırını ve `transfers_total{type="delegate",status="broadcast_failed"}` sayacını görür, alarm kuralı yok (backlog `StakeAccountUnmonitored`). Biçimi bozuk ayar — geçersiz ya da base58 olmayan adres, 2'den küçük ya da tam sayı olmayan kimlik — sidecar'ı açılışta durdurur (`STAKE_ACCOUNT_MISCONFIGURED`, proje sahibi kararı 2026-09-19). Hex adres (`41…`) TronWeb'in adres kontrolünden geçer ama düğüm `visible: true` okumalarında onu HTTP 200 + `Error` gövdesiyle reddeder ve kilit 0 görünürdü — bu yüzden yalnız base58 kabul edilir (Nile'da ölçüldü 2026-09-19). İki değişken yalnız blockchain sidecar'a gider (compose); açılan sidecar devretmenin sahibini, izin kimliğini ve iade tahmininin kilidi okuduğu hesabı tek bir `Energy delegation:` log satırında yazar — .env'den iki servise uzanan kablo bu satırı okuyan bir testle sabitli (#325 yeniden doğrulaması). **Ödeme enerjisi:** payout sıcak cüzdandan gider ve enerjiyi sıcak cüzdanın **kendi havuzundan** kullanır; kilit taşınınca o havuz boşalır, bu yüzden kilit hesabı ödeme payını (`N × 130.285 ÷ oran`) sıcak cüzdana bir kez ve kilitsiz devreder, devretme sürekli durur (proje sahibi kararı 2026-09-18 — kod yok, DEPLOY_RUNBOOK §C.2 adım 8). `getcandelegatedmaxsize` bu kalıcı devretmeyi düşer; depozit transferleri kalanı kullanır. **Ücret tahmini aynı hesabı okur** (§3.1a adım 3). **Ücret kilit hesabından çıkar:** ücretsiz bant 600 bayt/gün ve bir devret+geri al çifti 558 baytını yer; ötesi işlem başına ~0,28 TRX yakar (ölçülen 279.000 / 281.000 SUN). Boyutlandırma DEPLOY_RUNBOOK §C.2. Sidecar'ın **gönderdiği** TRX (hesap açma, bant ve yakma takviyesi, sabit yedek) aynı izin transferi reddettiği için sıcak cüzdandan çıkmaya devam eder — dört yolun dördü de testle ölçülür | 05 §3.3 |
| **Canlı doğrulama (kilit hesabı)** | **2026-09-18, Nile, derlenmiş kod:** kilit hesabı `TVqvXQ…EyQ6` (18 TRX ENERGY kilitli, pid 2 = sıcak cüzdan anahtarı) → sıcak cüzdana 1 TRX devretme blok **71.069.450** (ücret 279.000 SUN), geri alma blok **71.069.452** (281.000 SUN), **ayrı bloklar**. Enerji gerçekten ulaştı: alıcının `EnergyLimit` 7.370 → **7.444** → 7.370. Kilit hesabının bakiyesi 5.161.000 → 4.601.000 SUN, yani ücret **kilit hesabından** çıktı. Kontrol probu: aynı çağrı `ownerPermissionId` olmadan reddedildi. **Devretme yolu Nile'da uçtan uca koşulamadı** — 18 TRX'lik kilit, bir Nile TRC-20 transferinin istediği ~218 TRX'lik boyuta yetmiyor (#323'ün çarptığı aynı sınır); akışın kilit hesabını seçtiği sahte zincir testleri ve 13/13 bozmayla ölçüldü (düzeltme turu 2026-09-19: 21/21) | 2026-09-18 |
...
*Skinora — Integration Specifications v3.9*
