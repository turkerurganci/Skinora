...
3. **Platform ne kadar enerji getirebilir?** Payout'ta hot wallet doğrudan gönderdiği için havuzunun tamamı geçerlidir; eksik kalanı zincir yakar. İadede gönderen bir deposit adresidir ve iade, gönderimin izleyeceği **aynı planla** fiyatlanır (`DelegationPlanner`, aşağıdaki "Deposit adresi resource modeli"): hot cüzdan açığın **tamamını** devredebiliyorsa enerji kredisi tam, devredemiyorsa **sıfır** — kısmi kredi yoktur, çünkü gönderim kısmi devretme yapmaz. Kilitli bir hot cüzdanın büyük **havuzu** tek başına hiçbir şey ifade etmez; sayılan, bu transfer için devredilebilecek miktardır. Hiç TRX almamış bir deposit adresi 0 bant genişliği bildirir, ama gönderimden önce açıldığı için ücretsiz günlük banttan (600 bayt) yararlanır ve tahmin bunu hesaba katar.
...
| **Toplam** | **~14,6–21,2 TRX ≈ 4,9–7,1 $** | **~2 TRX ≈ 0,67 $** |
...
| **Plan** | Transfer simüle edilir, kontratın `consume_user_resource_percent` ayarı okunur, açık = arayanın payı − depozitin mevcut enerjisi. Devredilecek miktar = ⌈açık ÷ oran × 1,1⌉ TRX (en az 1 TRX). **Hep ya hiç:** sweeper'ın devredebildiği (`getcandelegatedmaxsize`) bu miktarı karşılıyorsa devredilir, karşılamıyorsa hiç devredilmez | `DelegationPlanner` |
| **Devretme yolu** | Devret → depozitin enerjisi gerçekten açığa ulaşana kadar bekle → transfer → geri al. Enerji gelmezse geri alınır ve yakma yoluna geçilir: kısmi enerjiyle gönderilen transfer zincirde başarısız olur ve arkasından hiçbir yedek çalışmaz | 2026-09-16 |
| **Yakma yolu** | Depozite yakacağı kadar TRX gönderilir (⌈açık × enerji ücreti × 1,1⌉ + gerekirse bant, eldeki bakiye düşülür), varışı beklenir, sonra transfer | 2026-09-16 |
| **Yedek** | Plan hiç hesaplanamazsa (zincir probu arızası) sabit `SWEEP_TRX_FALLBACK_SUN` (15 TRX) gönderilir — en pahalı transfer 13,03 TRX + açılış 1,1 TRX | 08 §3.3 |
| **Canlı doğrulama** | Derlenmiş akış Nile'da üç yolu uçtan uca geçti (2026-09-16): enerji gerekmeyen yol (`e7d71065…`), devretme — 5.000 enerji için 75 TRX (`f2f3079b…` / geri alma `94efa4e0…`), yakma — 1.604.240 SUN (`4ddb5440…`). Nile test USDT'si enerjiyi kendi ödediği için transferin **devredilen enerjiyi tükettiği** Nile'da ölçülemez; akışın zincir adımları ölçüldü | 2026-09-16 |
...
*Skinora — Integration Specifications v3.8*
