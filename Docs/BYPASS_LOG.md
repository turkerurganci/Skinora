...
| 2026-05-05 17:46 UTC | Türker urgancı | task/T58-dispute-system | `b238c8c` | [ci-failure] T58 CI fix — BlockchainTransaction CHECK constraint violation (ConfirmationCount + PaymentAddressId), pushing repaired test seed |
| 2026-05-05 20:24 UTC | Türker urgancı | task/T59-emergency-hold | `bcab472` | [ci-failure] T59 CI fix — CK_Transactions_FreezeActive (run 25399832698): FreezeAsync pre-pass T54 paterni; commit bcab472 root cause fix |
| 2026-05-06 21:14 UTC | Türker urgancı | task/T61-signalr-transactions-hub | `75fd8cc` | [ci-failure] T61 Docker layer-cache fix: prev run 25461173933 fail oldu çünkü Realtime modülü PR'da eklendi ama Dockerfile COPY listesinde yoktu (MSB3202). Tek satır fix push. |
| 2026-05-08 18:49 UTC | Türker urgancı | task/T63a-platform-public-endpoints | `82460cb` | [ci-failure] T63a SeedDataTests count assertion 37→41 düzeltmesi — önceki CI failure'ın root cause fix'i |
| 2026-05-11 19:37 UTC | Türker urgancı | task/T63b-retention-jobs | `fb31b1b` | [ci-failure] [ci-failure] T63b fix push — prior run 25692092667 Failed: 1 OrphanNotification.Transaction_Bound_Notifications fake transactionId FK constraint (SQL Server CI enforce, SQLite lokal etmedi); commit fb31b1b FK-bound test'i kaldırıyor |
| 2026-05-16 09:36 UTC | Türker urgancı | task/T69-bot-failover-capacity | `3c536aa` | [ci-failure] CI fixup push: prior run 25958428542 integration test failed (AuditLog DbSet missing in test model); this commit registers Platform module to fix it |
| 2026-05-16 09:44 UTC | Türker urgancı | task/T69-bot-failover-capacity | `36196a3` | [ci-failure] CI fixup push: prior run 25958615063 still had 1 of the 6 new BotEvent tests failing (MissingAccountName helper-fallback bug); this commit fixes the test to actually send null |
...
| 2026-06-13 21:06 UTC | Türker urgancı | task/T103b-2-bot-recovery | `6581b85` | [ci-failure] T103b-2: pushing fix for the failed Unit-test job (new AuditAction values + BotRecoveryStatus broke enum-count + audit-category-map guards); root cause resolved locally — Shared.Tests 356/356, Platform.Tests 106/106 |
| 2026-06-14 16:08 UTC | Türker urgancı | task/WP1-escrow-completion-payout | `822a41f` | [ci-failure] WP1 CI-fix: seed migration (önceki run pending-model-changes ile failed; bu commit düzeltmesi) |
| 2026-06-17 08:29 UTC | Türker urgancı | task/WP5-admin-dispute-resolution | `887998a` | [ci-failure] WP5 CI fix: Steam.Tests model-cache race (TestAssemblyModuleInitializer) — pushing root-cause fix over prior failed run |
| 2026-06-17 13:28 UTC | Türker urgancı | task/WP7-maintenance-toggle | `3a36f2a` | [ci-failure] WP7: pushing the fix for the failed Unit-test run (AuditAction enum parity + AuditLogCategoryMap mapping); local Unit-filter suite green |
