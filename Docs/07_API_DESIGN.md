...
**Versiyon: v4.2** | **Bağımlılıklar:** `02_PRODUCT_REQUIREMENTS.md`, `03_USER_FLOWS.md`, `04_UI_SPECS.md`, `05_TECHNICAL_ARCHITECTURE.md`, `06_DATA_MODEL.md`, `10_MVP_SCOPE.md` | **Son güncelleme:** 2026-09-16 (**§9.3 — `ABNORMAL_BEHAVIOR` pattern değerleri** — tipin iki üreticisi adlandırıldı: işlem öncesi `DORMANT_ACCOUNT_HIGH_VALUE` ve 02 §14.2 teslim etmeme yaptırımının hesap düzeyi `NON_DELIVERY_REPEAT` flag'i.) · 2026-09-16 (**§5.7 — cooldown 429'u `Retry-After` taşıyor** — `VERIFICATION_COOLDOWN` yanıtı kalan beklemeyi yalnız mesaj metninde veriyordu; §2.9'un her 429 için koyduğu başlık kuralına hizalandı, saniye yukarı yuvarlanmış ve mesajdaki sayıyla aynı kaynaktan.) · 2026-09-15 (**§7.3 — `steamAccountRemainingDays`** — uygunluk yanıtı, yalnız `STEAM_ACCOUNT_TOO_NEW` sebebiyle birlikte kalan gün sayısını döndürüyor. Alan kuralı ve örnek gövde eklendi; sayı yukarı yuvarlanır ve en az 1'dir.) · 2026-08-21 (**T135** — §7.5 yanıtına `buyerInventoryVisible?` eklendi: teslimat baseline’ının alınıp alınamadığı, yani 02 §9.2 envanter kanıtı yolunun bu işlemde açık olup olmadığı. Aynı olgu §7.6a confirm-ready yanıtında bir kez dönüyordu; kalıcı yüzeye çıkarılmasının sebebi yükümlülüğün kalıcı olması ve **yükümlülüğü taşıyan tarafın alıcı** olmasıdır — alıcı confirm-ready yanıtını hiç görmez, satıcı da ilk yenilemede kaybederdi. Kaynak `Transaction.BuyerBaselineCapturedAt`’in NULL olup olmaması (06 §3.5), yeni kolon yok. Kapı `payment` bloğuyla **aynı** damgadır (`SellerReadyConfirmedAt`), statü adı değil. Her iki tarafa döner, public/prospective görünümde ve onaydan önce hiç dönmez. Başka endpoint sözleşmesi değişmedi.) · 2026-08-19 (**T133a** — custodial kalıntı turu. **§8.1 bildirim kataloğu** 20 → **26 tip**, 06 §2.13 ile sıra dahil birebir: `ITEM_ESCROWED` → `PAYMENT_WINDOW_OPEN`, `TRADE_OFFER_SENT_TO_BUYER` → `DELIVERY_EXPECTED` (hedef taraf değişti), emekli `ITEM_RETURNED` + `ADMIN_STEAM_BOT_ISSUE` düştü, sekiz eksik tip eklendi. **§9 permission tablosu 11 → 12** (`MANAGE_SANCTIONS` eklendi) ve üç satırın uç atfı tamamlandı; §9.11'e parity çapası yazıldı. **§7.1** `active` sekmesi "terminal olmayan" tanımına çekildi, `cancelled`'a `REFUNDED` eklendi, EMERGENCY_HOLD'un sekme filtresine giremediği projeksiyon notuna yazıldı. **§7.5** koşullu bölümler tablosu kodun fiilî davranışına hizalandı — `steamTradeOfferUrl` artık PAYMENT_RECEIVED + satıcı görünümünde alıcının kendi trade URL'i, `payment`/`paymentEvents` eşiği `SELLER_CONFIRMED`, `deliveredBuyerAssetId` `ITEM_DELIVERED`'dan itibaren, `timeout.type` beş değeri sayıldı, `escrowBotAssetId` (kodda yok) silindi, `availableActions` örneğine v3.0'ın üç alanı eklendi. **`itemReturned`** üç örnek yanıttan (§7.5, §9.20, §9.22) kaldırıldı — DTO'larda yok. **§9.1 AD1** `steamAccounts` bloğu silindi. **§9.20/§9.22** iptal edilebilir state listeleri ve iade kuralları state machine'e hizalandı, item-iadesi bacakları kaldırıldı (02 §3.2). **§9.28/§9.29** kaldırılmış AD25/AD26 uçlarının tarihsel gövdeleri silindi — §9.10 AD10 deseniyle yalnız kaldırma notu kaldı (gövdeler emekli statü, entity ve audit aksiyonu vaat ediyordu). §1.1 endpoint sayıları 67 → **82** (+2 SignalR), §2.1 konvansiyon örneği ve §3.1/§3.2 izlenebilirlik satırları güncellendi; altbilgi başlıkla hizalandı. Endpoint davranışı değişmedi.) · 2026-08-19 (**T132** — §9 permission tablosu ve §9.11 `availablePermissions` kataloğu **14 → 12**: `VIEW_STEAM_ACCOUNTS` ve `MANAGE_STEAM_RECOVERY` kaldırıldı. İkisi de hiçbir endpoint'te enforce edilmiyordu ve koruyacakları uçlar (§9.10 AD10, §9.28 AD25, §9.29 AD26) v3.0'da zaten kaldırılmıştı — katalogda kalmaları var olmayan bir ekran için rol tanımlanabilmesine yol açıyordu (02 §15, 05 §3.2). **§11.2 RT2 admin grubu kataloğundan `AdminBotStatusChanged` olayı kaldırıldı** — backend'de payload / publisher portu / event sabiti silindiği için sözleşmenin vaat ettiği olay artık yayınlanamıyordu; liste üçten ikiye indi ve kalıcı kayıt satırından emekli `PlatformSteamBot.Status` atfı düştü (T132 doğrulaması, bulgu B1). Endpoint sözleşmesi başka yerde değişmedi.) · 2026-08-17 (**T131** — §9.30: **AD29'un item-iade bacağı kaldırıldı** — `ItemRefundToSellerRequestedEvent` v3.0'da (T117) silinmişti, sözleşme onu vaat etmeye devam ediyordu; AD29 body'sine `overrideReason? (20..2000)` ve yeni **400 `OVERRIDE_REASON_REQUIRED`** eklendi (kanıtlanmış teslimatta `BUYER_FAVOR` zorunlu kılar), kapı sırası not edildi (durum guard'ları gerekçe kontrolünden önce — zaten reddedilecek bir karar için gerekçe istenmez), SELLER_FAVOR maddesine bekletilen teslimat imzasının admin kararıyla timeout'a **serbest bırakılması** yazıldı. AD28 cevabına `resolutionOverrideReason?` ve `buyerFavorRequiresOverride` (**her zaman** döner) eklendi; `deliveredItemName` notuna **yüzey kuralı** yazıldı — alan admin ekranında `transaction.itemName`'in altında çizilir, alan yoksa satır **hiç çizilmez**.) · 2026-08-17 (**T130** — AD28 (`GET /admin/disputes/:id`) cevabına `deliveredItemName?` eklendi: yanlış item vakasında gerçekten gelen item'ın adı (06 §3.11), yalnız WRONG_ITEM otomatik eskalasyonunda ve tek yeni sınıf geldiğinde döner. Admin bunu `transaction.itemName` ile yan yana görür ve karşılaştırmayı elle yapmak zorunda kalmaz (02 §10.1). **Not:** alanın admin ARAYÜZÜNDE yüzeye çıkarılması T130 doğrulamasının N2 bulgusudur ve **T131**'e kabul kriteri olarak yazılmıştır — sözleşme hazır, ekran değil.) · 2026-08-17 (T129 düzeltme turu — yeni **§9.22b AD32** `POST /admin/transactions/:id/clear-settlement` (yetki `MANAGE_DISPUTES`): eskale edilmiş mutabakatı satıcı lehine kapatır, statü değiştirmez; §9.3 `flagDetail` tablosuna `DELIVERY_REVERSED` (12 alan) ve `SANCTIONS_MATCH` satırları eklendi.) · 2026-08-16 (T128 — §7.2 hata listesine 422 `ITEM_ALREADY_LISTED` eklendi ve 02 §2.3'ün "aynı item ikinci kez listelenemez" kuralı uç sözleşmesinde normatif not oldu: kapı 06 §5.1 filtered unique index'inin filtresini birebir aynalar, terminal statüler kapsam dışıdır, eşzamanlı iki isteğin ürettiği indeks çakışması da aynı koda çevrilir. Başka endpoint sözleşmesi değişmedi.) · 2026-08-14 (T125 — §9.8 kategori listesine `delivery_verification` eklendi ve anahtar sayısı 60'a çekildi (satır T123 döneminde 58'de kalmıştı). Endpoint sözleşmesi değişmedi.) · 2026-08-13 (T123 — §7.6a confirm-ready: hata listesine 422 `INVENTORY_PRIVATE` eklendi (satıcı envanteri gizliyse item "yok" sayılamaz — §7.2/§6.1 ile aynı üç değerli ayrım), yanıta `buyerInventoryVisible` alanı ve `paymentDeadline`'ın kaynağı yazıldı, baseline'ın sayım tabanlı olduğu ve okunamazsa kolonların NULL kaldığı normatif not oldu.) · 2026-08-11 (T121 — §7.2 hata listesi envanter okumasının 08 §2.3'teki üç sonucunu ayrı raporlayacak şekilde tamamlandı: `ITEM_NOT_IN_INVENTORY` (kod üretiyordu, listede yoktu) korundu, 422 `INVENTORY_PRIVATE` ve 503 `STEAM_UNAVAILABLE` eklendi — ikisi de §6.1'in envanter sözlüğünden, yeni kod icat edilmedi.) · 2026-08-10 (T119a — §7.6 accept ucu v3.0 alanları: `steamTradeUrl` sahiplik doğrulaması (partner ↔ alıcının kendi SteamID64'ü) ve Steam erişilemediğinde fail-closed 503 `STEAM_UNAVAILABLE` hata listesine eklendi; §5.1 `GET /users/me` yanıtına salt-okunur `steamTradeUrl` eklendi — §7.6 ön-doldurma kaynağı.)
...
| Kural | Örnek |
|-------|-------|
| Küçük harf, kebab-case | `/audit-logs`, `/sanctions/addresses` |
...
| S05 | T1, U2, N2 | — |
| S06 | S1, T3, T4 | T2 |
| S07 | T5, T5a | T6, T6a, T6b, T7, T8, T9, T10, T11 |
| S08 | U1 | U3, U4, A5, A6 |
| S09 | U5 | — |
...
| §2.3 Satıcı hazırlık onayı | T6a, T5 (real-time: RT1) |
...
| §3.5 Item teslimi ve doğrulanması | T6b, T5 (real-time: RT1) |
...
429 `VERIFICATION_COOLDOWN` yanıtı, §2.9'daki rate limit 429'u gibi **`Retry-After` header'ı** taşır: kalan bekleme saniye cinsinden, yukarı yuvarlanmış (cooldown sürerken `0` olmaz). Aynı sayı insan için `error.message` içinde de yazılıdır; iki değer aynı kaynaktan üretilir.
...
| Tab | Status'ler |
|-----|-----------|
| `active` | Terminal olmayan tüm statüler: CREATED, ACCEPTED, SELLER_CONFIRMED, PAYMENT_RECEIVED, ITEM_DELIVERED, FLAGGED |
| `completed` | COMPLETED |
| `cancelled` | CANCELLED_TIMEOUT, CANCELLED_SELLER, CANCELLED_BUYER, CANCELLED_ADMIN, REFUNDED |

> **Terminal statüler:** COMPLETED, CANCELLED_TIMEOUT, CANCELLED_SELLER, CANCELLED_BUYER, CANCELLED_ADMIN, REFUNDED. `active` sekmesi bunların dışında kalan tüm statüleri kapsar; üç sekmenin birleşimi 12 statünün tamamını verir ve kesişimleri boştur.
>
> **EMERGENCY_HOLD projection notu:** Backend'de EMERGENCY_HOLD bir `TransactionStatus` değeri değildir — herhangi bir aktif statü üzerine uygulanan dondurma overlay'idir (`Transaction.IsOnHold` bayrağı + `TimeoutFreezeReason = EMERGENCY_HOLD`, 05 §4.5, 06 §3.5). Bu yüzden **sekme filtresinde yer almaz**: filtre gerçek `Status` kolonu üzerinde çalışır, hold'daki satır alttaki gerçek statüsünün sekmesinde (`active`) listelenir. API response'ta ise `status: "EMERGENCY_HOLD"` olarak **computed status** sunulur; bu projection frontend'in hold durumunu ayrı bir durum gibi işlemesini sağlar. Backend gerçek statü `PreviousStatusBeforeHold` alanında korunur (03 satır 38, 05 §4.5).
...
`reasons` içinde `STEAM_ACCOUNT_TOO_NEW` varsa yanıta **tek sayısal alan** eklenir (08 §2.2a):
...
```json
{
  ...
  "cancelCooldown": { "active": false, "expiresAt": null },
  "newAccountLimit": { "isNewAccount": true, "current": 0, "max": 2 },
  "reasons": ["STEAM_ACCOUNT_TOO_NEW"],
  "steamAccountRemainingDays": 4
}
```

| Alan | Tip | Kural |
|------|-----|-------|
| `steamAccountRemainingDays` | `int?` | Steam'in 15 günlük takas beklemesinden kalan gün. **Yalnız** `reasons` içinde `STEAM_ACCOUNT_TOO_NEW` varken yazılır; diğer tüm durumlarda alan yanıtta **hiç yer almaz**. Yukarı yuvarlanır ve en az `1` olur — hâlâ engelliyken "0 gün kaldı" yanlış olurdu. Bu listedeki diğer engeller beklemekle değil bir eylemle biter (kısıtlı hesapta 5 USD harcama gibi), dolayısıyla onlara gün sayısı iliştirilmez |
...
| `deliveredBuyerAssetId` | ITEM_DELIVERED'dan itibaren — teslimat doğrulaması sırasında yazılır (06 §8.4); alıcının envanterinde tespit edilen asset ID. Public view'de `null` |
| `steamTradeOfferUrl` | **(v3.0)** PAYMENT_RECEIVED + **satıcı** görünümünde — alıcının kabul sırasında verdiği kendi Steam trade URL'i (`Transaction.BuyerTradeUrl`, §7.6). Satıcı item'ı doğrudan bu adrese gönderir; platform trade offer oluşturmaz (02 §2.2 adım 6, 04 §7.3 "Steam'de Trade Offer Gönder" butonu). Diğer statülerde, alıcı görünümünde ve public view'de `null` |
| `buyerInventoryVisible` | **(v3.0, T135)** Satıcının hazırlık onayından itibaren — teslimat baseline'ının alınıp alınamadığı (03 §2.3 adım 3). Kapı `payment` bloğuyla **aynı** kilometre taşı damgasıdır (`SellerReadyConfirmedAt`), statü adı değil; bu yüzden pencere açıldıktan sonra iptal edilen işlemlerde de görünür kalır. Kaynağı `Transaction.BuyerBaselineCapturedAt`'in NULL olup olmamasıdır (06 §3.5) — ayrı bir kolon değil, aynı kararın projeksiyonu. **Her iki tarafa da** döner; public/prospective görünümde ve satıcı onayından **önce** alan hiç dönmez (okuma henüz yapılmadı — "bilinmiyor", "görünür" değil) |

> **Not (`buyerInventoryVisible`, T135):** Aynı olgu §7.6a'nın confirm-ready yanıtında da bir kez döner. Burada tekrar edilmesinin sebebi olgunun **kalıcı** olmasıdır: `false` iken 02 §9.2'nin envanter kanıtı yolu işlemin sonuna kadar kapalıdır ve teslimat yalnız alıcının "Teslim Aldım" onayıyla kanıtlanabilir. Yükümlülüğü taşıyan taraf **alıcıdır** ve alıcı confirm-ready yanıtını hiç görmez — tek yüzey o yanıt olsaydı, ekranı yenileyen satıcı da uyarıyı kaybederdi. 04 §7.3 ACCEPTED ve 03 §3.5 notlarının FE dayanağı bu alandır.

> **Not:** Steam trade sonrası asset ID değişir — v3.0'da tek bir trade vardır (satıcı → alıcı) ve alıcının envanterinde oluşan yeni asset ID `deliveredBuyerAssetId` alanında audit ve dispute doğrulaması için döndürülür (06 §8.4).

**`paymentEvents` (SELLER_CONFIRMED+, 04 §7.3 S07 banner'ları):**
```json
[
  {
...
3a. **Alıcının Steam takas uygunluğu doğrulanır** (v3.2, 08 §2.2a) — hold süresi "takas ne kadar bekletilir" sorusunu cevaplar, "bu hesap takas edebilir mi" sorusunu değil: **limited** bir hesap ve Steam'in **15 günlük** beklemesindeki bir hesap da 0 saniye döndürür. İkisi de ayrı kodla reddedilir (`STEAM_ACCOUNT_LIMITED` / `STEAM_ACCOUNT_TOO_NEW`), çünkü çareleri farklıdır (5 USD harcama / beklemek / MA açmak). Okunamayan cevap yine 503 `STEAM_UNAVAILABLE` — bilgi yokluğu olumsuz bulguyla aynı koda çöktürülmez

**Hatalar:** 409 `INVALID_STATE_TRANSITION`, 403 `STEAM_ID_MISMATCH`, 409 `ALREADY_ACCEPTED`, 400 `VALIDATION_ERROR`, 400 `INVALID_WALLET_ADDRESS`, 400 `INVALID_TRADE_URL` *(v3.0 — format veya sahiplik)*, 403 `MOBILE_AUTHENTICATOR_REQUIRED` *(v3.0)*, 403 `STEAM_ACCOUNT_LIMITED` *(v3.2 — 08 §2.2a)*, 403 `STEAM_ACCOUNT_TOO_NEW` *(v3.2 — 08 §2.2a)*, 503 `STEAM_UNAVAILABLE` *(v3.0 — MA ya da takas uygunluğu doğrulanamadı, tekrar denenebilir)*, 403 `SANCTIONS_MATCH`, 403 `WALLET_CHANGE_COOLDOWN_ACTIVE`, 403 `ACCOUNT_FLAGGED` (hesap-flag accept gate, 02 §14.0 — WP4a), 403 `NOT_A_PARTY` (OPEN_LINK'te satıcı kendi listesini kabul edemez, 02 §6.2)
...
**Hatalar:** 409 `INVALID_STATE_TRANSITION`, 403 `NOT_A_PARTY`, 409 `ITEM_NO_LONGER_AVAILABLE` *(item envanterde yok veya artık tradeable değil)*, 422 `INVENTORY_PRIVATE` *(v3.0 — satıcının profili gizli, item doğrulanamadı)*, 403 `BUYER_MOBILE_AUTHENTICATOR_INACTIVE`, 403 `BUYER_STEAM_ACCOUNT_LIMITED` *(v3.2 — alıcının hesabı kısıtlı, takas ona ulaşamaz; 08 §2.2a)*, 403 `BUYER_STEAM_ACCOUNT_TOO_NEW` *(v3.2 — alıcı Steam'in 15 günlük beklemesinde)*, 503 `STEAM_UNAVAILABLE`
...
| `BUYER_ACCEPTED` | Satıcı | Alıcı işlemi kabul etti | transaction |
| `PAYMENT_WINDOW_OPEN` | Alıcı | Satıcı hazır, ödeme yapabilirsin (v3.0: eski `ITEM_ESCROWED`) | transaction |
| `PAYMENT_RECEIVED` | Satıcı | Ödeme doğrulandı | transaction |
| `DELIVERY_EXPECTED` | Satıcı | Ödeme alındı, item'ı alıcıya gönder (v3.0: eski `TRADE_OFFER_SENT_TO_BUYER`; hedef taraf alıcıdan satıcıya değişti) | transaction |
...
| `ADMIN_PAYMENT_FAILURE` | Admin | Satıcıya ödeme gönderim hatası (tekrarlayan) | transaction |
| `ADMIN_PLATFORM_OUTAGE` | Admin | Platform kesintisi tespit edildi (02 §3.3) | null |
| `EMERGENCY_HOLD_APPLIED` | Her ikisi | Admin işlemi acil dondurma uyguladı (§9.21) | transaction |
| `EMERGENCY_HOLD_RELEASED` | Her ikisi | Admin acil dondurmayı kaldırdı (§9.22) | transaction |
| `INSUFFICIENT_PAYMENT` | Alıcı | Eksik ödeme tespit edildi (02 §4.4) | transaction |
| `OVERPAYMENT_REFUNDED` | Alıcı | Fazla ödeme iade edildi | transaction |
...
| `MANAGE_FLAGS` | Flag onayla/reddet (AD4, AD5) + hesabı askıya al / askıyı kaldır (AD20, AD21) |
...
| `EMERGENCY_HOLD` | İşlemleri acil dondurma/kaldırma (AD19b, AD19c) + kullanıcı bazlı toplu dondurma (AD19d) |
| `VIEW_DISPUTES` | İtiraz kuyruğunu görüntüle (AD27, AD28) |
| `MANAGE_DISPUTES` | İtirazları çöz (AD29) + eskale edilmiş mutabakatı satıcı lehine kapatma (AD32) |
| `MANAGE_SANCTIONS` | Sanctions listesi yönet (AD22, AD23, AD24) |
...
| ABNORMAL_BEHAVIOR | `{ pattern, description }` — `pattern` üreticiyi adlandırır: `DORMANT_ACCOUNT_HIGH_VALUE` (işlem öncesi, 02 §14.4) · `NON_DELIVERY_REPEAT` (hesap düzeyi, 02 §14.2 — `description` penceredeki işlemleri id, tür ve tarihle sayar) |
...
> **Not (v3.0, P2P geçişi):** Katalog **14 → 12** yetkiye indi. `VIEW_STEAM_ACCOUNTS` ve `MANAGE_STEAM_RECOVERY` kaldırılmıştır (T132) — platform Steam hesabı işletmediği için koruyacakları S18 ekranı, emanet item listesi ve recovery kuyruğu yoktur (02 §15, 05 §3.2; kaldırılan uçlar §9.10 AD10, §9.28 AD25, §9.29 AD26). Kaldırılan iki yetki hiçbir endpoint'te enforce edilmiyordu; katalogda kalmaları yalnızca var olmayan bir ekran için rol tanımlanabilmesine yol açıyordu.
>
> **Parity çapası (T133a):** Bu liste kod kataloğunun (`PermissionCatalog.All`, `backend/src/Modules/Skinora.Admin/Application/Permissions/PermissionCatalog.cs`) birebir aynasıdır — anahtar, etiket ve sıra dahil. Katalogda bir yetki eklenir veya kaldırılırsa üç doküman nüshası **aynı commit'te** güncellenir: §9.11 (tek doğru kaynak), §9 permission tablosu ve 04 §8.8 yetki matrisi. Doğrulama: üç tablonun anahtar kümesi ile `PermissionCatalog.All` arasındaki fark boş olmalıdır.
>
...
**İptal edilebilir state'ler:** CREATED, ACCEPTED, SELLER_CONFIRMED, PAYMENT_RECEIVED, FLAGGED (02 §7).

**İptal edilemez:** ITEM_DELIVERED (item alıcıya teslim edilmiş ve mutabakat penceresi işliyor — standart iptal/para iadesi uygulanamaz; alıcı lehine tek çıkış AD29 dispute çözümüdür, §9.30), COMPLETED, CANCELLED_*, REFUNDED. Hold'daki işlem de reddedilir: hold bir statü değil, statünün üzerine binen bir bayraktır (`isOnHold`) — önce AD19c ile kaldırılır (§9.22).

**İade kuralları:** Yalnızca para hareket eder. Ödeme alınmışsa (PAYMENT_RECEIVED) alıcıya iade edilir — fiyat + komisyon − gas fee (02 §4.4); ödeme alınmamışsa iade yoktur. **Item iadesi diye bir işlem yoktur:** platform item'ı hiçbir aşamada tutmaz, iptal anında item nerede duruyorsa orada kalır (02 §3.2). Yanıttaki `itemReturned` alanı v3.0'da kaldırıldı (§7.7 ile aynı gerekçe).
...
|-------------------|------------------|----------------|
| CREATED, ACCEPTED, SELLER_CONFIRMED | Evet | Ödeme henüz alınmamıştır → iade doğmaz; item zaten satıcının envanterindedir |
| PAYMENT_RECEIVED | Evet | Ödeme alıcıya iade edilir (fiyat + komisyon − gas fee). Item'a dokunulmaz — iptal anında nerede duruyorsa orada kalır (02 §3.2) |
| FLAGGED | Evet | İade yok — FLAGGED yalnız işlem oluşturma anında set edilir, o noktada ödeme henüz alınmamıştır (05 §4.2) |
| ITEM_DELIVERED | **Hayır** | Item zaten alıcıda ve mutabakat penceresi işliyor — standart iptal/para iadesi uygulanamaz. CANCEL reddedilir (422 `CANNOT_CANCEL_AT_DELIVERY_STAGE`), yalnızca RESUME izinli; alıcı lehine çıkış yolu AD29 dispute çözümüdür (§9.30) |
...
Permission: `VIEW_DISPUTES`. Response: `{ id, type, status, systemCheckResult?, deliveredItemName?, userDescription?, adminId?, adminNote?, resolutionOverrideReason?, buyerFavorRequiresOverride, resolvedAt?, createdAt, updatedAt, transaction { id, status, itemName, price, stablecoin, isOnHold, hasActiveDispute, seller, buyer? } }`. **Hatalar:** 404 `DISPUTE_NOT_FOUND`.

> **`deliveredItemName` (T130):** yanlış item vakasında gerçekten gelen item'ın adı (06 §3.11). Yalnız WRONG_ITEM otomatik eskalasyonunda ve tek bir yeni sınıf geldiğinde döner — admin `transaction.itemName` ile bunu yan yana görür ve karşılaştırmayı elle yapmak zorunda kalmaz (02 §10.1). Diğer tüm dispute'larda alan yoktur. **Yüzey (T131):** alan admin çözüm ekranında `transaction.itemName`'in hemen altında çizilir; alan yoksa satır **hiç çizilmez** — yokluk "bilinmiyor" değil "bu vaka değil" demektir.

> **`buyerFavorRequiresOverride` (T131):** bu dispute `BUYER_FAVOR` ile çözülecek olsaydı AD29 ayrıca bir `overrideReason` ister miydi (02 §10.4)? Sunucuda hesaplanır ve **her zaman** döner (opsiyonel değil). Amacı, ekranın gerekçeyi admin karar verirken sorabilmesidir — kuralı istemcide yeniden türetmek, aynı predicate'i iki dilde iki kez yazmak ve sürüklenmeye açık bırakmak olurdu. Bugünkü değeri `transaction.status == ITEM_DELIVERED`'dır.

> **`resolutionOverrideReason` (T131):** geçmiş bir kararın kanıta rağmen verildiğini gösteren kayıtlı gerekçe (06 §3.11). Yalnız o kararlarda döner.

#### AD29 — `POST /admin/disputes/:id/resolve`
Permission: `MANAGE_DISPUTES`. Body: `{ outcome: "SELLER_FAVOR" | "BUYER_FAVOR", adminNote (1..2000), overrideReason? (20..2000) }`.
- **SELLER_FAVOR:** dispute → `RESOLVED_FOR_SELLER`; `HasActiveDispute` temizlenir (başka aktif dispute yoksa) → WP1 satıcı payout ITEM_DELIVERED'da devam eder. Transaction state geçişi yok. **Yanlış-teslimat imzası nedeniyle bekletilen bir işlemde (02 §9.2) bu karar, teslimat timeout'unun kilidini açar (T131):** sonraki tarama turu satırı artık Held'de tutmaz ve süresi dolmuş pencere olağan seyrini izler → `CANCELLED_TIMEOUT` + alıcıya iade (03 §4.4, §6.4). Karar öncesinde iptal yasaktı çünkü 02 §9.2 **sessiz** iptali yasaklar; admin baktıktan sonra iptal sessiz değildir. Sistemin kendi otomatik çözümüyle kapanmış (`CLOSED`) bir itiraz bu kilidi **açmaz**.
- **BUYER_FAVOR:** dispute → `RESOLVED_FOR_BUYER`; işlem `AdminResolveRefund` tetikleyicisiyle `REFUNDED`'a geçer; alıcı ödediyse `PaymentRefundToBuyerRequestedEvent` (WP2) yayınlanır. **Item iadesi bacağı yoktur** (T131 — platform item tutmaz, 02 §3.2): karar yalnızca parayı hareket ettirir; item kararın verildiği anda neredeyse orada kalır, fiziksel geri-alma ayrı manuel/WP6 sürecidir.
- **`overrideReason` (T131 — 02 §10.4, 03 §6.4):** işlem `ITEM_DELIVERED` durumundayken verilen `BUYER_FAVOR` kararında **zorunludur** (≥20 karakter, trim'lenir); o durumda eksikse istek reddedilir. `ITEM_DELIVERED` teslimatın kanıtlanmış olması demektir (duruma girişin ön koşulu 02 §9.2 kanıtıdır), dolayısıyla karar platformun kendi bulgusunu geri çevirir ve zararı geri alınamaz biçimde satıcıya devreder. Diğer tüm durumlarda alan **yok sayılır ve saklanmaz** — dolu kolonun tek bir anlamı olsun diye (06 §3.11). Kapı **duruma** bakar, kanıt bayraklarına değil: bayrağa bağlamak yükümlülüğü Steam'in o an okunabilir olmasına bağlardı. Kapının cevabı AD28'de `buyerFavorRequiresOverride` olarak önceden verilir.
- Her iki sonuç: `AdminId`/`AdminNote`/`ResolvedAt` set; `DISPUTE_RESOLVED` audit (gerekçe audit `NewValue`'suna da yazılır — kolon okunduğu yer, audit değiştirilemediği yer, 06 §3.20); `DisputeResolvedEvent` → buyer + seller `DISPUTE_RESULT` bildirimi. Tüm yan etkiler tek `SaveChanges` ile atomik.

**Response (200) `data`:** `{ id, status, transactionStatus, resolvedAt, buyerRefunded }`.

**Hatalar:** 400 `VALIDATION_ERROR` (adminNote eksik / > 2000 / geçersiz outcome / overrideReason > 2000), 400 `OVERRIDE_REASON_REQUIRED` (kanıtlanmış teslimatta BUYER_FAVOR, gerekçe yok veya <20 karakter — T131), 404 `DISPUTE_NOT_FOUND`, 409 `DISPUTE_NOT_ESCALATED` (yalnız ESCALATED çözülebilir), 409 `TRANSACTION_ON_HOLD` (önce AD19c ile hold release), 409 `INVALID_STATE_TRANSITION`.

> **Kapı sırası (T131):** durum guard'ları (`ESCALATED`, hold) `overrideReason` kontrolünden **önce** çalışır — zaten reddedilecek bir karar için admin'den gerekçe istenmez.
...
**Bu endpoint kaldırılmıştır (v3.0, P2P geçişi).** Platform item tutmadığı için bir bota mahsur kalabilecek item ve dolayısıyla recovery kuyruğu yoktur (02 §15, 06 §3.10a). `MANAGE_STEAM_RECOVERY` yetkisi, `BotRecoveryItem` entity'si ve S18 admin ekranı (04 §8.7) da kaldırılmıştır; backend yüzeyi T132'de, sidecar tarafı T133'te silindi.

> Alt bölüm numarası bilinçli korundu — §9.29 ve sonrası referanslarının kayması engellendi.

### 9.29 AD26 — `PATCH /admin/steam-accounts/recovery/:id`

**Bu endpoint kaldırılmıştır (v3.0, P2P geçişi)** — AD25 ile aynı gerekçe: recovery kuyruğu diye bir kavram kalmamıştır. `MANAGE_STEAM_RECOVERY` yetkisi, `BotRecoveryItem` entity'si (06 §3.10a) ve `BOT_RECOVERY_UPDATED` audit aksiyonu (06 §2.19) da kaldırılmıştır.

> Alt bölüm numarası bilinçli korundu — §9.30 ve sonrası referanslarının kayması engellendi.
...
**Admin grubu (WP9):** Aşağıdaki **iki** olay yalnızca **admin grubuna** (`admins`) push edilir — admin (`role ∈ {admin, super_admin}`) bağlantıları RT2'ye bağlanırken otomatik bu gruba katılır; admin olmayan istemciler bu payload'ları almaz (`Clients.All` yerine grup kapsamı). Kalıcı kayıt: ilgili AuditLog satırı (`RECONCILIATION_MISMATCH` / `HOT_WALLET_THRESHOLD_BREACHED`).

> **Not (v3.0, P2P geçişi):** Bu liste T132'ye kadar **üç** olaydı. `AdminBotStatusChanged` (T69 — platform Steam bot'u RESTRICTED/BANNED/OFFLINE'a geçti veya havuzdan çıkarıldı) **kaldırılmıştır**: platform Steam hesabı işletmediği için bildirilecek bot durumu, kalıcı kaydı tutan `PlatformSteamBot` tablosu (T117'de düştü) ve backend'de payload / publisher portu / SignalR event sabiti yoktur (02 §15, 05 §3.2).
