...
> **Launch kapısı (T129):** Geri alma imzası ölçülmemiş bir çıkarımdır (T122 gerçek bir rollback gözleyemedi). Bu yüzden otomatik iade `settlement.reversal_auto_refund_enabled` ayarının arkasındadır ve launch'ta **kapalıdır**: imza kaydedilir, admin'e eskale edilir, para parkta bekler. Kaydedilen imza **yapışkandır** — ayrılmayı gözlemlemiş bir eskalasyon açıkken sonraki turun "item duruyor" okuması ödemeyi serbest bırakmaz; kararı insan verir. Teslimat kanıtı kapısının (§9.2) ikizidir; prosedür DEPLOY_RUNBOOK §I.
