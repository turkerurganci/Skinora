...
| ✓ | ~~P2P-BotCodeArchive~~ | **KAPANDI (T133, 2026-08-19).** Bot custody kodunun arşiv işaretçisi — üç halkanın üçü de silindi ve hepsinin git işaretçisi aşağıda. **1) Domain — T117 `82bff4d`:** `TradeOffer`/`PlatformSteamBot`/`BotRecoveryItem` entity'leri, bot seçimi, dispatch, recovery, `SteamWebhooksController` (Steam modülü 35→11 dosya). **2) Backend kalıntısı — T132 `eb0e49d`** (PR [#247](https://github.com/turkerurganci/Skinora/pull/247)): 4 ölü `AuditAction` bot değeri + `AuditLogCategoryMap` girdileri, `VIEW_STEAM_ACCOUNTS`/`MANAGE_STEAM_RECOVERY`, `AdminBotStatusChanged`, `/api/v1/webhooks/steam` middleware dalı + `SteamSharedSecret`. **3) Sidecar — T133** (PR [#248](https://github.com/turkerurganci/Skinora/pull/248)): `sidecar-steam/src/bot/`, `TradeOfferService`/`TradeOfferMonitor`/`trade/types.ts`, `src/webhook/` dizininin tamamı, `link-authenticator.cjs`, `02-register-bot.sql` (16 dosya / ~4.600 satır) + üç npm bağımlılığı. **İşaretçi olarak PR numarası kullanıldı, squash sha'sı değil:** sha merge anında doğar, yani satır kendi kapanışını hiçbir zaman yazamıyordu (T132'de de aynı sebeple boş kaldı); `git log --grep "(#248)"` squash commit'i sha'dan bağımsız bulur. Geri dönüş senaryosu yok (eski model Steam kuralı nedeniyle çalışmıyor) ama cooldown'ı olmayan bir oyuna genişlerken referans olabilir | doc-drift | 11 T132/T133 |
