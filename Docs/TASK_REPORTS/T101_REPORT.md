...
- **K11 — İptal modal'ı "iade bilgisi" göstermiyor (S3, 04 §8.5 / 03 §8.7):** Cancel modal yalnız onay sorusu (`confirm.cancelTx`) + sebep textarea içeriyordu; "neyin kime iade edileceği" özeti yoktu. **✅ ÇÖZÜLDÜ (PR [#155](https://github.com/turkerurganci/Skinora/pull/155), 2026-06-06):** `FlagActionModal`'a opsiyonel `infoBlock` slot'u + `TransactionDetailView`'da `tx.status`-türevli ileri-bakışlı iade özeti (ITEM_ESCROWED+ → item satıcıya; PAYMENT_RECEIVED+ → ödeme alıcıya fiyat+komisyon−gas fee; aksi → iade yok) AD19/AD19c CANCEL tablosuyla hizalı + `cancelRefund` 4-locale (parity 863×4). Bağımsız 3-ajan adversarial doğrulama allCorrect (state haritası + i18n + regresyon temiz).
