...
| 7 | **Steam'in ham gövdesi saklanabilir** (AC6 metni) | `steamcommunity` sayfalama + `assets × descriptions × asset_properties` birleştirmesini **kendi içinde** yapıp `CEconItem[]` döndürüyor; kütüphanenin **public API'ı** ham gövdeyi vermiyor | ✗ **KIRIK** (public API düzeyinde) — aşağıya bakın. *Düzeltme (doğrulama bulgusu F1): satır önce "ham JSON hiçbir noktada dışarı verilmiyor" diyordu; `httpRequest` seam'i üzerinden **mümkündü**, yapılmama sebebi imkânsızlık değil kapsam/kişisel-veri kararı* |
