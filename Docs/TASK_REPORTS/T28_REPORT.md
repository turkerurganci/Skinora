...
- `IntegrationTestBase` `EnsureCreatedAsync` yerine `MigrateAsync` çağırıyor — her integration test'i artık prod-equivalent şema inşa yolundan geçiyor.
...
- **Karar — IntegrationTestBase MigrateAsync geçişi:** T26 seed testleri dahil tüm integration test'ler artık migration yolundan gidiyor. SQLite-backed unit test'ler (OutboxTests, EfCoreGlobalConfigTests, API middleware testleri) kendi DbContext'ini farklı yoldan setup ettiği için etkilenmiyor.
