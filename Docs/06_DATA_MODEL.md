...
**Versiyon: v6.16** | **Bağımlılıklar:** `02_PRODUCT_REQUIREMENTS.md`, `03_USER_FLOWS.md`, `05_TECHNICAL_ARCHITECTURE.md`, `09_CODING_GUIDELINES.md`, `10_MVP_SCOPE.md` | **Son güncelleme:** 2026-09-17 (**§3.17 — #323 doğrulama düzeltmesi** — anahtar sayısı `blockchain.sweep_energy_delegation_sun` silindikten sonra "67"de kalmıştı → **66** (tablo seed'le anahtar anahtar yeniden karşılaştırıldı, birebir). `blockchain.sweep_trx_fallback_sun` açıklaması yeni anlamına çekildi: "delegation başarısızsa" değil, **kaynak planı hiç hesaplanamazsa** gönderilen sabit TRX; seed açıklaması migration `SweepTrxFallbackDescription` ile güncellendi.) · 2026-09-16 (**§3.17 — `blockchain.sweep_energy_delegation_sun` kaldırıldı** — hibrit enerji kararıyla devredilecek miktar transfer başına hesaplanır hâle geldi; hiçbir şeyin okumadığı sabit ayar satırı migration `RemoveSweepEnergyDelegationSetting` ile silindi.) · 2026-09-16 (**§3.17 — teslim etmeme yaptırımının üç ayarı** — `non_delivery_window_days` (30), `non_delivery_flag_count` (2), `non_delivery_suspend_count` (3) eklendi; 0 ya da okunamaz değerin kuralı varsayılana düşürmek yerine kapattığı yazıldı. **Doğrulama düzeltmesi (PR #322):** üç satır tablonun ortasına girmişti ve anahtar sayısı "63"te kalmıştı; satırlar seed sırasına (65–67) taşındı, #315'ten beri eksik olan `blockchain.max_charged_gas_fee_usdt` (64) satırı eklendi, sayı **67** oldu — tablo seed'le anahtar anahtar birebir. §2.19 `USER_BANNED` satırına ikinci üretici (otomatik askı, aktör SYSTEM) yazıldı.) · 2026-08-20 (**T139** — §2.16 `MonitoringStatus` tablosu güncellendi: `ACTIVE`in iki anlamı (kararı D4 — tahsis edildi / sidecar'da kurulu, ayrım kolonda değil registry'de) ve `STOPPED`in yeni mutlu-yol yazarı (pencere kapanışı: sweep veya terminal statü) kayda geçti; §3.7'nin `MonitoringStatus` satırı bu nota ve 08 §3.4'ün yaşam döngüsü tablosuna yönlendiriyor.) · 2026-08-19 (**T133a** — §2.19 `AuditAction` tablosu kod enum'uyla birebir **29 satıra** tamamlandı; on iki değerin satırı yoktu: dört `FRAUD_FLAG_*` (T54), `RECONCILIATION_MISMATCH` (T76), `COLD_WALLET_TRANSFER_INITIATED` + `HOT_WALLET_THRESHOLD_BREACHED` (T77), `SANCTIONS_LIST_ADDRESS_ADDED` / `_REMOVED` (T82), `MAINTENANCE_MODE_CHANGED` (WP7), `TIMEOUT_AUTO_EXTENDED` + `PLATFORM_OUTAGE_DETECTED` (WP16). Boşluk T132 öncesinden geliyordu ve tek yönlüydü — eksik olan DOKÜMANDI. `Grup` değerleri `AuditLogCategoryMap` ile birebir alındı (tek asimetri: `COLD_WALLET_TRANSFER_INITIATED` = **Fon**). Tablonun altına parity'yi normatif kılan kaynak notu eklendi ve altbilgi başlıkla hizalandı. Şema değişikliği yok.) · 2026-08-18 (**T131 doğrulaması tur 2 — bloke etmeyen bulgu N1**: §2.10 `DisputeStatus` tablosu `RESOLVED_FOR_*`'a hâlâ koşulsuz **"Terminal."** diyordu, oysa aynı turda yazılan N2c yeniden eskalasyonu o statüyü geri döndürüyor ve bu **varsayılan yoldur** — ilk-tur imzasında karşılaştırma anı `now` olduğu için mevcut **her** karar imzadan önce sayılır. Tabloya istisna işaretlendi ve §2.10'a yeniden eskalasyon notu eklendi (kural 02 §10.2'de; `AdminNote`/`AdminId` korunur, `ResolvedAt` temizlenir, ikinci dispute **satırı** hâlâ yasak, `CLOSED` istisnanın dışında). Şema değişikliği **yok** — yalnız bayat normatif ifade düzeltildi.) · 2026-08-18 (**T131 düzeltme turu** — doğrulama bulgusu B1: §3.5'e `TimeoutReleasedByAdminRulingAt` (datetime, NULL) eklendi ve §3.1 `SuccessfulTransactionRate` formülünün paydası `CANCELLED_TIMEOUT[TimeoutReleasedByAdminRulingAt IS NULL]` olarak **bölündü**; sorumluluk haritasının teslimat satırına da aynı koşul yazıldı. `DeliveryReversedAt`'in ikizi: tek status'ün iki üreticisi kusur hakkında zıt şey söylüyor ve ayrım satırda taşınıyor. Damgalı satır **ne itibara ne cooldown'a** girer (02 §14.2) — kaydettiği şey admin'in satıcıyı akladığı karardır, sınıfı `CANCELLED_ADMIN` ile aynıdır (02 §13). **Şema:** 1 additive nullable kolon, migration `T131_TimeoutReleasedByAdminRulingAt`; seed/CHECK/index yok.) · 2026-08-17 (**T131** — §3.11'e `ResolutionOverrideReason` (string(2000), NULL) eklendi: yalnız teslimatı **kanıtlanmış** bir işlemde alıcı lehine verilen kararda dolar; override'ı olmayan bir kararda alan gönderilse bile **saklanmaz**, böylece dolu kolon her zaman tek bir şey demektir ve istisna kaydı aranabilir kalır. Genişlik `AdminNote` ile aynı — yanındakinden dar bir gerekçe alanı gerekçeyi nota iterdi. Aynı bölümün **WP5 notundaki item-iade bacağı kaldırıldı** (v3.0'da platform item tutmaz, 02 §3.2). **Şema:** 1 additive nullable kolon, migration `T131_DisputeResolutionOverrideReason`.) · 2026-08-17 (**T130** — §3.5'e `BuyerBaselineClassIds` (string(max), alıcı envanterinin **tüm** benzersiz `ClassId` kümesi) eklendi ve **kırpılmaz** olarak işaretlendi: düşen her sınıf sonradan "yeni geldi" diye okunur, bu da satıcı hakkında uydurulmuş bir iddiadır. `BuyerBaselineAssetIds` satırı düzeltildi — o kolon **bu sınıfa ait** asset listesidir, "yanlış item tespiti" görevi ona yüklenemez (sınıf-kapsamlı baseline farklı sınıfı göremez). `BuyerBaselineCapturedAt` satırına dört baseline kolonunun **birlikte yazıldığı** invariant'ı yazıldı. §3.11'e `DeliveredItemName` (string(200)) — yalnız WRONG_ITEM otomatik eskalasyonunda ve tek yeni sınıfta dolar; `SystemCheckResult` içine gömülmez çünkü o metin alıcının dilindedir ve admin o dili okumayabilir. **Şema:** 2 additive nullable kolon, migration `T130_WrongItemEvidenceColumns`.) · 2026-08-17 (T129 ikinci düzeltme turu — §3.5 `SettlementEscalationReason` satırına **güç sırası** yazıldı: yapışkanlık kolonun değerine bağlı olduğu için değerin kendisi de korunur, kod yalnız güçlendirilebilir (`SETTLEMENT_UNREADABLE` = `SETTLEMENT_NO_DELIVERY_REFERENCE` < `SETTLEMENT_AMBIGUOUS_DEPARTURE` < `SETTLEMENT_REVERSAL_GATED`) ve eşit güçteki iki kod birbirinin üstüne de yazmaz — prosedürde ayrışırlar (DEPLOY_RUNBOOK §I.3 ↔ §I.5), gözlemde ayrışmazlar. Şema değişikliği **yok**. Gerekçe: sıra olmadan tek bir `Inconclusive` tur yapışkanlığı kendi kendine iptal ediyordu (bulgu B4).) · 2026-08-17 (T129 düzeltme turu — §3.5'e `SettlementEscalationReason` + `SettlementClearedByAdminId`, §2.11'e `DELIVERY_REVERSED`, §2.19'a `SETTLEMENT_CLEARED_ADMIN`; §3.1 girişi ile §8.2 tetikleyicisi geri alma dalıyla genişletilip **çelişkileri giderildi**; §3.17 anahtar sayısı 58 → **63** ve eksik `blockchain.payout_gas_fee_estimate_usdt` satırı eklendi; launch kapısı atfı §H → **§I**.) · 2026-08-16 (T129 — §3.5'e iki kolon (`SettlementCheckedAt`, `SettlementEscalatedAt`) ve `PayoutEligibleAt`'ın ITEM_DELIVERED **giriş guard'ı** olduğu notu; §3.1 oran formülünün paydasına `REFUNDED[DeliveryReversedAt NOT NULL]` (mutabakat geri alması satıcıya yazılır, admin dispute iadesi hariç kalır); §3.17'ye üç `Settlement` ayarı. Şema: 2 nullable kolon + 3 seed satırı, migration `T129_SettlementCheckColumns`.) · 2026-08-14 (T125 — yeni entity §3.5a `DeliveryEvidenceCapture` (append-only teslimat kanıt kaydı) ve §8'e `delivery.inventory_evidence_auto_release_enabled` launch kapısı satırı eklendi. Şema: 1 yeni tablo, migration `T125_DeliveryEvidenceCapture`.) · 2026-08-13 (T123 — §8 SystemSetting tablosunda iki custodial anahtar yeniden adlandırıldı: `trade_offer_seller_timeout_minutes` → `seller_confirm_timeout_minutes`, `trade_offer_buyer_timeout_minutes` → `delivery_timeout_minutes`. Şema değişikliği yok; migration satır `Id`'leri sabit tutan bir `UpdateData`'dır.) · 2026-08-10 (T119a — §3.5 zorunlu-field matrisine `BuyerTradeUrl` için DB CHECK istisnası notu eklendi: kolon nullable kalır, invariant `HasFieldsForAccepted` guard'ında korunur. Şema değişikliği yok.)
...
| `RESOLVED_FOR_SELLER` | Admin satıcı lehine çözdü (WP5 — işlem onaylanır, satıcı payout devam eder). Terminal — **tek istisnası** aşağıdaki T131 yeniden eskalasyon notudur. |
| `RESOLVED_FOR_BUYER` | Admin alıcı lehine çözdü (WP5 — işlem REFUNDED, alıcıya iade). Terminal — aynı istisnaya tabidir, ama pratikte erişilemez (aşağıdaki not). |
...
> **Not (T131 — yeniden eskalasyon, `RESOLVED_FOR_*`'ın terminalliğinin tek istisnası):** Karardan **sonra** platformun kendisi yeni bir olgu gözlemlerse — teslimat turu, kararın ardından item'ın satıcıdan ayrıldığını tespit ederse — **aynı satır** `ESCALATED`'a döner: `SystemCheckResult` yeni bulguyla yazılır, `ResolvedAt` **temizlenir** (`CK_Disputes_Resolved_ResolvedAt` ikisini eşler), `AdminNote` + `AdminId` **korunur** ve önceki karar değiştirilemez `DISPUTE_RESOLVED` audit kaydında kalır (§3.20). Kural 02 §10.2'de; kapıyı yalnız **platformun yeni gözlemi** açar, kullanıcı itirazı açmaz. İkinci bir dispute **satırı** hâlâ yasaktır (`UQ_Disputes_TransactionId_Type`) — bu, aynı satırın yeni bir olguyla yeniden ele alınmasıdır. Kod kolu `RESOLVED_FOR_*`'ın ikisini de kapsar, ama pratikte erişilebilir olan `RESOLVED_FOR_SELLER`'dır: alıcı lehine karar işlemi `REFUNDED`'a taşır ve teslimat tarayıcısı yalnız `PAYMENT_RECEIVED` sorgular. `CLOSED` bu istisnanın **dışındadır** — orada bir insan kararı yoktur, korunacak veya gözden geçirilecek bir hüküm de yoktur.
...
| `DELIVERY_REVERSED` | Mutabakat sonu kontrolünde satıcının trade'i geri aldığının tespiti (02 §4.5.1, T129) — **hesap düzeyi** (`ACCOUNT_LEVEL`) işaret, çünkü bulgu işlemin değil kişinin hakkındadır ve 02 §14.2 yaptırımı tekrarı sayar. Ayrı tip olmasının sebebi bu sayımdır: `ABNORMAL_BEHAVIOR` torbasına atılsaydı "kaçıncı geri alma" sorusu admin kuyruğunda cevaplanamazdı |
...
| Değer | Açıklama |
|-------|----------|
| `ACTIVE` | Adres platformun aktif izleme yaşam döngüsünde — tahsiste yazılır, ödeme penceresi açıldığında sidecar'a kurulur (3 sn polling) |
...
| `STOPPED` | İzleme sonlandı — ödeme penceresi kapandı (depozit sweep edildi veya işlem terminal, T139) ya da gecikmeli izleme 30 günü doldurdu |

> **Kaynak:** 05 §3.3 — gecikmeli ödeme izleme takvimi. Aktif izlemenin tetikleyicileri (kim kurar, kim durdurur) 08 §3.4'te tablolanmıştır.

> **`ACTIVE` iki durumu birden taşır (T139 kararı D4).** "Adres tahsis edildi ama pencere henüz açılmadı" (`CREATED`/`ACCEPTED`) ve "adres sidecar'da kurulu" (`SELLER_CONFIRMED` ve sonrası) aynı değerle temsil edilir; ayrım kolonda değil sidecar registry'sinde durur ve `EnsurePaymentMonitorJob` her dakika işlemin statüsünden yeniden türetir. Enum'a üçüncü bir değer **eklenmedi**: migration + kardeş projelerdeki enum parity testleri + `IX_PaymentAddresses_MonitoringStatus_Active` filtre listesi maliyetine karşılık hiçbir davranış kazanılmıyor, çünkü kolonun anlamı zaten "bu adres aktif izleme yaşam döngüsünde"dir ve `STOPPED` o döngünün bittiğini söyler.
...
| `USER_BANNED` | Admin | Kullanıcı engelleme (askıya alma). İki üreticisi var: admin (07 §9.26 AD20, aktör ADMIN) ve 02 §14.2 teslim etmeme yaptırımının otomatik askısı (aktör SYSTEM). İkisi aynı yazıcıdan geçer; askıyı kimin koyduğu `ActorType`'tan okunur |
...
| `FRAUD_FLAG_CREATED` | Admin | Otomatik fraud tespiti flag açtı — hesap seviyesi (`ACCOUNT_LEVEL`) veya işlem seviyesi (`TRANSACTION_PRE_CREATE`) kapsamda; aktör SYSTEM, EntityType = `FraudFlag` (02 §14.0, 07 §9.2 AD2, T54) |
| `FRAUD_FLAG_APPROVED` | Admin | Admin flag'i inceleyip işleme devam kararı verdi (false positive) — `TRANSACTION_PRE_CREATE` kapsamında işlem `FLAGGED` → `CREATED` olur (07 §9.4 AD4, T54) |
| `FRAUD_FLAG_REJECTED` | Admin | Admin flag'i reddetti (fraud doğrulandı) — `TRANSACTION_PRE_CREATE` kapsamında işlem `FLAGGED` → `CANCELLED_ADMIN` olur (07 §9.5 AD5, T54) |
| `FRAUD_FLAG_AUTO_HOLD` | Admin | Hesap seviyesi flag kullanıcının aktif işlemlerine otomatik `EMERGENCY_HOLD` uyguladı — her işlem için ayrı satır, EntityType = `Transaction` (02 §14.0, 07 §9.21, T54) |
...
| `SETTLEMENT_CLEARED_ADMIN` | Admin | Admin eskale edilmiş mutabakatı satıcı lehine kapattı (07 §9.22b AD32, 02 §4.5.1, T129) — `SettlementVerifiedAt` + `SettlementClearedByAdminId` damgalanır, statü değişmez |
| `RECONCILIATION_MISMATCH` | Güvenlik | Günlük mutabakat job'ı zincir ↔ defter farkı tespit etti — her (scope, token) uyuşmazlığı için bir satır (scope ∈ `DepositAddress` / `HotWallet` / `ColdWallet`), aktör SYSTEM; kalıcı kayıt admin'e giden realtime bildirimden bağımsızdır (05 §3.3, T76) |
| `COLD_WALLET_TRANSFER_INITIATED` | Fon | Admin hot → cold operasyonel transfer başlattı — `ColdWalletTransfer` defter satırıyla aynı `SaveChanges` içinde yazılır (06 §3.22, 05 §3.3, T77) |
| `HOT_WALLET_THRESHOLD_BREACHED` | Güvenlik | Periyodik hot wallet bakiye monitörü admin tanımlı eşiğin aşıldığını (`Upper`) veya altına düşüldüğünü (`Lower`) tespit etti — aktör SYSTEM, EntityType = `HotWallet` (05 §3.3, T77) |
| `SANCTIONS_LIST_ADDRESS_ADDED` | Güvenlik | Admin sanctions listesine yeni adres ekledi — EntityType = `SanctionedAddress`; eşleşen adresin kaynak ve gerekçe kaydı bu satırdadır (07 §9.24 AD23, 02 §21.1, T82) |
| `SANCTIONS_LIST_ADDRESS_REMOVED` | Güvenlik | Admin sanctions listesindeki adresi deaktive etti — satır silinmez, `IsActive = false` olur (07 §9.25 AD24, 02 §21.1, T82) |
| `MAINTENANCE_MODE_CHANGED` | Admin | Admin bakım/kesinti moduna girdi veya çıktı — EntityType = `Maintenance`, EntityId yeni bakım tipi; eski/yeni dört `platform.maintenance.*` ayarı ve dondurulan/serbest bırakılan işlem sayısı kaydedilir (07 §9.31 AD30/AD31, 05 §4.4, WP7) |
| `TIMEOUT_AUTO_EXTENDED` | Admin | Yeniden başlatma kurtarması kesinti eşiğini aştı ve aktif timeout'lar otomatik uzatıldı — kurtarma turu başına tek özet satır (kesinti süresi, uzatılan işlem sayısı, yeniden planlanan ödeme job'ları), aktör SYSTEM, EntityType = `SystemHeartbeat` (05 §4.4, WP16) |
| `PLATFORM_OUTAGE_DETECTED` | Güvenlik | Periyodik sağlık probu bir sidecar'ın (`STEAM` / `BLOCKCHAIN`) ardışık hata eşiğini aştığını (`DEGRADED`) veya toparlandığını (`RECOVERED`) tespit etti — durum geçişi başına tek satır, aktör SYSTEM, EntityType = `PlatformHealth`; yalnız `ADMIN_PLATFORM_OUTAGE` bildirimi üretir, işlemleri dondurmaz (05 §4.4, 02 §3.3, WP16) |

> **Kaynak:** Kod enum'u `AuditAction` (`backend/src/Skinora.Shared/Enums/AuditAction.cs`) — tablo **29 değerin tamamını** listeler, alt küme değildir; enum'a değer eklendiğinde bu tabloya da satır eklenir. `Grup` kolonu 07 §9.19 AD18 `category` filtresinin birebir karşılığıdır: Fon = `FUND_MOVEMENT`, Admin = `ADMIN_ACTION`, Güvenlik = `SECURITY_EVENT`; eşleme `AuditLogCategoryMap`'te tutulur ve haritalanmamış bir değer çalışma zamanında hata verir. Alanın entity tanımı için bkz. §3.20.
...
> **İtibar skoru:** `CompletedTransactionCount`, `SuccessfulTransactionRate` ve `CreatedAt` (hesap yaşı) alanlarından oluşur. Denormalized field'lar işlem tamamlandığında, iptal olduğunda veya mutabakat sonu kontrolü geri alma tespit ettiğinde (REFUNDED, `DeliveryReversedAt NOT NULL` — T129) güncellenir.
>
...
> - `CANCELLED_TIMEOUT` da **bölünerek** dahil edilir (T131 doğrulaması bulgu B1, proje sahibi kararı D6, 2026-08-18). Yanlış-teslimat imzası nedeniyle bekletilen bir teslimat penceresi, admin dispute'u karara bağladığı için serbest bırakıldığında (03 §6.4, `TimeoutReleasedByAdminRulingAt NOT NULL`) ortaya `PAYMENT_RECEIVED → CANCELLED_TIMEOUT` satırı çıkar — yani aşağıdaki sorumluluk haritasının **satıcıya** yazdığı satır. Bu, admin'in `RESOLVED_FOR_SELLER` ile **açıkça akladığı** satıcıdır: satır sayılırsa 03 §6.4'ün "satıcının kaydına kusur yazılmaz" kuralı çiğnenir ve ceza **kalıcı** olur (itibarın admin düzeltme yüzeyi yoktur, skor satırlardan yeniden hesaplanır). Ayırt edici alan `TimeoutReleasedByAdminRulingAt`'tir; onu yalnız teslimat timeout turunun admin-serbest-bırakma kolu yazar. Sınıf gerekçesi CANCELLED_ADMIN ile aynıdır (02 §13): platform kararı, kullanıcı kusuru değil. Aynı dışlama **cancel cooldown** sayımında da geçerlidir (02 §14.2) — iki tüketici ayrışamaz, cooldown satıcıyı işlem açmaktan tamamen alıkoyar.
...
| `SettlementEscalatedAt` | datetime | NULL | Mutabakat kontrolünün işlemi admin'e devrettiği an (T129). Dört sebep: envanter `settlement.unreadable_escalation_hours` boyunca okunamadı; teslimat ölçülebilir bir referans bırakmadı (ne `DeliveredBuyerAssetId` ne alıcı baseline'ı üretildi — bu sebep eşik **beklemeden ilk turda** eskale eder, çünkü karar girdisi sonradan da doğmaz, T129 düzeltme turu); item alıcıdan ayrıldı ama satıcıya döndüğü kanıtlanamadı; ya da geri alma imzası otomatik iade kapısı kapalıyken oluştu. Hangi sebeple devredildiği `SettlementEscalationReason`'da tutulur. Idempotency işareti — admin bir kez haberdar edilir, her turda değil |
| `SettlementEscalationReason` | string(64) | NULL | Eskalasyonun gerekçe kodu (T129): `SETTLEMENT_UNREADABLE`, `SETTLEMENT_NO_DELIVERY_REFERENCE`, `SETTLEMENT_AMBIGUOUS_DEPARTURE`, `SETTLEMENT_REVERSAL_GATED` — `SettlementReviewReasons` sabitleri. Enum kolonu **değildir**: kodlar outbox event'inde zaten string olarak taşınır. İki işi vardır. (1) Triyaj: admin kuyruğu eskalasyon sınıflarını ayırt edebilir. (2) **Yapışkanlık:** ayrılmayı *gözlemlemiş* bir gerekçe yazılıysa (`SETTLEMENT_AMBIGUOUS_DEPARTURE` veya `SETTLEMENT_REVERSAL_GATED`) sonraki tur "item alıcıda duruyor" dese bile `SettlementVerifiedAt` damgalanmaz — para insan kararına kadar parkta kalır. Aksi hâlde item'ın alıcıya geri dönmesi (ya da envanterin yeniden görünür olması) gözlemlenmiş ayrılmayı sessizce silerdi. Yapışkanlık kolonun **değerine** bağlı olduğu için değerin kendisi de korunur: kod **yalnız güçlendirilebilir**, düşürülemez. Güç sırası `SETTLEMENT_UNREADABLE` = `SETTLEMENT_NO_DELIVERY_REFERENCE` (0) < `SETTLEMENT_AMBIGUOUS_DEPARTURE` (1) < `SETTLEMENT_REVERSAL_GATED` (2); yeni bir tur yalnız kendi kodu **kesin olarak daha güçlüyse** yazar, eşit veya daha zayıfsa kayıtlı gerekçe korunur (yalnız log). Eşit güçteki iki kod prosedürde ayrışır (DEPLOY_RUNBOOK §I.3 ↔ §I.5), gözlemde ayrışmaz — bu yüzden biri diğerinin üstüne de yazmaz. Sıra olmadan tek bir `Inconclusive` tur `SETTLEMENT_REVERSAL_GATED`'i `SETTLEMENT_UNREADABLE`'a çevirip yapışkanlığı kendi kendine iptal ediyordu (T129 ikinci düzeltme turu, bulgu B4) |
| `SettlementClearedByAdminId` | guid | NULL | Mutabakatı admin kapattıysa hangi admin (T129, AuditAction `SETTLEMENT_CLEARED_ADMIN`). **Hiçbir para kapısı bu kolonu okumaz** — kapılar `SettlementVerifiedAt`/`DeliveryReversedAt` çiftine bakar; bu alan yalnız kanıt ve triyaj içindir. FK tanımlı değildir |
| **Concurrency** | | | |
...
| `MonitoringStatus` | int | NOT NULL | Enum: MonitoringStatus — yaşam döngüsü ve `ACTIVE`in iki anlamı için §2.16 notu, tetikleyiciler için 08 §3.4 |
...
| `ResolutionOverrideReason` | string(2000) | NULL | Admin, teslimatı **kanıtlanmış** bir işlemde (`Transaction.Status = ITEM_DELIVERED`) alıcı lehine karar verdiyse bu kararın gerekçesi (T131 — 02 §10.4, 03 §6.4). Diğer tüm çözümlerde NULL; override'ı olmayan bir kararda alan gönderilse bile **saklanmaz**, böylece dolu kolon her zaman tek bir şey demektir — platformun kendi teslimat kanıtına rağmen verilmiş karar — ve istisna kaydı aranabilir kalır. `AdminNote` ile aynı genişlik: ikisi de admin metnidir ve yanındakinden dar bir gerekçe alanı, gerekçeyi nota iterdi (03 §6.4'ün "ayrıca kayda geçirilir" ifadesinin yasakladığı karışım) |
| `IsDeleted` | bool | NOT NULL, DEFAULT 0 | Soft delete flag |
| `DeletedAt` | datetime | NULL | Silinme zamanı |
| `CreatedAt` | datetime | NOT NULL | |
| `ResolvedAt` | datetime | NULL | Çözüm zamanı |
...
>
> **Admin çözümü (WP5 — 03 §6.4):** ESCALATED dispute'u admin `RESOLVED_FOR_SELLER` (işlem onaylanır → satıcı payout `ITEM_DELIVERED`'da devam eder) veya `RESOLVED_FOR_BUYER` (işlem `REFUNDED` → alıcıya **ödeme** iadesi) ile kapatır; `AdminId`/`AdminNote`/`ResolvedAt` set edilir. **Item iadesi bacağı yoktur** (T131 — v3.0'da platform item tutmaz, 02 §3.2); alıcı lehine karar yalnızca parayı hareket ettirir. Teslimatı kanıtlanmış bir işlemde alıcı lehine karar ayrıca `ResolutionOverrideReason` ister (02 §10.4). Emergency hold altındaki işlem önce AD19c ile release edilmelidir.
>
> **State-dependent CHECK constraint'ler:**
...
| `settlement.reversal_auto_refund_enabled` | Settlement | bool | false | Geri alma tespitinde otomatik iade açık mı (T129 launch kapısı, `delivery.inventory_evidence_auto_release_enabled`'ın ikizi). false iken imza admin'e eskale edilir, parayı kendiliğinden hareket ettirmez — bkz. DEPLOY_RUNBOOK §I |
| `blockchain.max_charged_gas_fee_usdt` | Monitoring | decimal | 10.0 | Kullanıcıdan kesilebilecek gas fee üst sınırı (USDT). Runtime tahmin bu değeri aşarsa tahmin **reddedilir** (kırpılmaz) ve statik fallback kesilir. Sağlıklı bir tahmini değil bozuk bir tahmini yakalamak içindir; 0 = tavan kapalı (08 §3.1a) |
| `non_delivery_window_days` | Fraud | int | 30 | Teslim etmeme yaptırımının yuvarlanan penceresi (gün) — 02 §14.2. Sayılan olaylar ve istisnaları 02 §14.2'de. 0 ya da okunamaz değer kuralı **kapatır** (varsayılana düşmez) |
| `non_delivery_flag_count` | Fraud | int | 2 | Pencere içindeki kaçıncı teslim etmeme olayında satıcı hesabına `ABNORMAL_BEHAVIOR` flag'i (`flagDetail.pattern = NON_DELIVERY_REPEAT`, 07 §9.3) yazılacağı. 0 = kural kapalı |
| `non_delivery_suspend_count` | Fraud | int | 3 | Pencere içindeki kaçıncı teslim etmeme olayında satıcı hesabının **otomatik** askıya alınacağı — süresiz, admin kaldırır. 0 = kural kapalı |
...
| User | SuccessfulTransactionRate | İşlem COMPLETED, CANCELLED veya mutabakat sonu geri almasıyla REFUNDED (`DeliveryReversedAt NOT NULL`, T129) olduğunda — sadece sorumlu tarafın skoru güncellenir (§3.1 formül detayı). Geri alma dalında refresh terminal statü flush edildikten **sonra** çağrılır, aksi hâlde payda geri alınan işlemi saymadan hesaplanır | Eventual (cross-module) |
