...
const SERVICE_NAME = "skinora-steam-sidecar";
...
const lokiTransport = pino.transport({
  ...
  options: {
    ...
    // Drop log push errors silently — Loki being down must not crash the sidecar.
    silenceErrors: true,
  },
});
...
/**
 * Build a request-scoped child logger that carries the correlationId for an
 * incoming HTTP request. Reads `X-Correlation-Id` header (case-insensitive) or
