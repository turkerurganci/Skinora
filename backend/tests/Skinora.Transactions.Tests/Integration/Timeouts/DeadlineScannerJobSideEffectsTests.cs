...
/// whose buyer refund T124 severed and T127 restored — now behind the
/// verification round that decides whether the cancellation happens at all. The
/// Payment phase is the one leg driven by a per-transaction Hangfire job rather
/// than this scanner (05 §4.4); it is covered by
...
public class DeadlineScannerJobSideEffectsTests : IntegrationTestBase
{
    ...
    private DeadlineScannerJob CreateSut(DeliveryTimeoutDecision deliveryDecision = DeliveryTimeoutDecision.Held) =>
    ...
    /// <summary>
    /// T127 — a delivery timeout the verification round authorised reaches the
    /// publisher again: the cancellation notice plus the buyer refund 03 §4.4
    /// step 4 owes. There is no item leg — the item never left the seller.
    /// </summary>
    [Fact]
    public async Task Delivery_Timeout_Publishes_Notification_And_PaymentRefund()
    {
        ...
        await CreateSut(DeliveryTimeoutDecision.Cancel).ScanAndRescheduleAsync();

        var timedOut = Assert.Single(_outbox.Published.OfType<TransactionTimedOutEvent>());
        Assert.Equal(TimeoutPhase.Delivery, timedOut.Phase);

        var refund = Assert.Single(_outbox.Published.OfType<PaymentRefundToBuyerRequestedEvent>());
        Assert.Equal(buyer.Id, refund.BuyerId);
        Assert.Equal(TimeoutTestFixtures.ValidWallet, refund.BuyerRefundAddress);
    ...
    /// <summary>
    /// The invariant the buyer refund hangs on: a round that concludes nothing
    /// publishes nothing. Money must not move on an expired clock alone — the
    /// seller may have delivered and simply never been confirmed (02 §9.2).
    /// </summary>
    [Fact]
    public async Task Held_Delivery_Timeout_Publishes_Nothing()
    {
        var nowUtc = _clock.GetUtcNow().UtcDateTime;
        var buyer = await TimeoutTestFixtures.AddBuyerAsync(Context);
        var transaction = TimeoutTestFixtures.NewTransaction(
