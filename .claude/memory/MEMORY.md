...
- **T29 (⏳ branch `task/T29-steam-openid-auth`, yapım bitti, doğrulama bekliyor):** Steam OpenID 2.0 authentication — `Skinora.Auth/Application/SteamAuthentication/` (OpenId validator + profile client + access/refresh token gen + user upsert + return URL sanitizer + login audit) + `AuthController` (GET `/auth/steam` + `/auth/steam/callback`) + `SteamOpenIdSettings`. Geo-block (T30/T83) + sanctions (T82) pipeline hook'u stub (`AllowAllGeoBlockCheck`, `NoMatchSanctionsCheck`) — interface sabitlendi, DI swap ile gerçek impl devralır. 32 unit + 6 integration test ✓, 105 API.Tests regresyon ✓, migration yok. Test öğrenimi: xUnit parallel fixture + Hangfire `JobStorage.Current` process-global race → factory `HangfireBypassFactory` yerine plain `WebApplicationFactory<Program>`'dan türetildi, Hangfire services scrublandı, `Hangfire:DashboardEnabled=false`, `NoopBackgroundJobScheduler` stub.
- **Next:** T29 doğrulama → T30 (ToS kabul, yaş gate, geo-block).
