...
- **T29 (⏳ branch `task/T29-steam-openid-auth`, S1 düzeltme uygulandı, re-doğrulama bekliyor):** Steam OpenID 2.0 authentication — `Skinora.Auth/Application/SteamAuthentication/` (OpenId validator + profile client + access/refresh token gen + user upsert + return URL sanitizer + login audit) + `AuthController` (GET `/auth/steam` + `/auth/steam/callback`) + `SteamOpenIdSettings`. Geo-block (T30/T83) + sanctions (T82) pipeline hook'u stub (`AllowAllGeoBlockCheck`, `NoMatchSanctionsCheck`) — interface sabitlendi, DI swap ile gerçek impl devralır. **S1 Düzeltme (2026-04-21, commit `e6e102a`):** `RefreshTokenGenerator.IssueAsync` artık `Token = SHA256(plainText)` hex saklar (06 §3.3 L441 uyumu); cookie plain text kalır (`GeneratedRefreshToken.PlainTextToken`), DB yalnız hash görür. Integration test DB.Token ≠ cookie + `SHA256(cookie) == DB.Token` assert eder. T32 refresh path incoming cookie → hash lookup'ı devralacak. **Test infra follow-up (commit `f99d565`):** CI run 24739083342 AuthSteamEndpointTests 6/6 FAIL "Cannot create a DbSet for 'RefreshToken'..." — kök neden: EF `IModelSource` modeli DbContext tipine göre process-statik cache'ler; IntegrationTestBase türevi test (OutboxTests vb.) Program.cs olmadan DbContext kurarsa ve WebApplicationFactory testinden önce çalışırsa eksik model cache'lenir. CI'da shared MSSQL pre-warm sıralamayı ters çevirince latent flaky maskesi düştü. Fix: `Skinora.API.Tests/TestAssemblyModuleInitializer.cs` `[ModuleInitializer]` 10 modülü assembly yüklenirken kaydeder. CI run 24740092503 @ `3583c17` ✓ PASS. BYPASS_LOG: T11.2 Layer 2 1× entry (ci-failure, f99d565 fix push).
- **Next:** T29 re-doğrulama (yeni validate chat) → T30 (ToS kabul, yaş gate, geo-block).
