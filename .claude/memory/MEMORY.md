...
- **Implementation:** **F1 Gate Check ✓ PASS** (2026-04-20, PR #44 squash `b37f484`, tag `phase/F1-pass`). T01-T28 + T11.1 + T11.2 + T11.3 tümü ✓. **F2 aktif: T29 ✓ PASS + T30 ✓ PASS (2026-04-21) + T31 ⏳ yapım bitti (2026-04-23).** Sırada T32 (Refresh token yönetimi) — T31 doğrulama sonrası.
...
- **T31 (⏳ yapım bitti 2026-04-23, branch `task/T31-steam-reverify-authenticator`, PR pending):** Steam re-verify + authenticator kontrolü — 3 yeni endpoint (`POST /auth/steam/re-verify`, `GET /auth/steam/re-verify/callback`, `POST /auth/check-authenticator`). Yeni servisler: `IReAuthPipeline` + `ReAuthPipeline` (OpenID URL builder re-verify return_to ile, 48-byte random token + SHA-256 hash), `IReAuthStateProtector` (IDataProtector, 10 dk state cookie `skinora_oid_rv`), `IReAuthTokenStore` (Redis prod `GETDEL` atomic + InMemory test, TTL 5 dk single-use), `IReAuthTokenValidator` (T34 `X-ReAuth-Token` header için), `IMobileAuthenticatorCheck` + `StubMobileAuthenticatorCheck` (conservative `{active:false, setupGuideUrl:...}` — gerçek sidecar impl T64–T69). `ISteamOpenIdValidator.ValidateAsync(..., expectedReturnTo, ...)` overload + `SteamOpenIdUrlBuilder.Build(settings, returnTo)` overload (login/re-verify assertion cross-replay koruması). `SteamOpenIdSettings.ReVerifyReturnToUrl` required property + appsettings güncellendi. Referrer-Policy: same-origin callback response'unda. 12 yeni unit (ReAuthPipeline 6, InMemoryReAuthTokenStore 4, ReAuthTokenValidator 3) + 7 yeni integration (AuthReVerifyEndpoint) PASS; regression AuthSteam + TosAccept temiz. Migration yok (Redis-backed). Rapor: `Docs/TASK_REPORTS/T31_REPORT.md`.
- **Next:** T31 validate chat'i (ayrı chat) — PASS sonrası T32 (Refresh token yönetimi).
